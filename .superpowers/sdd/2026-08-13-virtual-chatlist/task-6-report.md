...
## Ответ на находку Task 7 — `items` ломали `useShouldAnimate`

**Дефект.** `useShouldAnimate` (`components/virtual/useShouldAnimate.ts`) сравнивает
элементы старого и нового списка ПО ССЫЛКЕ — дословный порт `prev.indexOf(item)`
(`verticalVirtualList.tsx:143-172`). `items` пересоздавали обёртки `{id, index,
value}` на каждом пересчёте, поэтому пересечение «видимые до»/«видимые сейчас»
было ВСЕГДА пустым: ветка `!visiblePrevAndNow.length` давала
`allChangedTheSameAmount = false`, `shouldAnimate` всегда `true`,
`onScrollShift` не звался никогда. Компенсация равномерного сдвига не работала —
при появлении чата над видимой областью дёргался весь экран. Это критерий
приёмки №4 спеки этапа.

**Контракты не конфликтуют** (проверил оба перед правкой):

- `useShouldAnimate` требует, чтобы ссылка на обёртку ЖИЛА, пока живёт строка;
- докблок пропа `items` ядра (`DeferredSortedVirtualList.tsx:64-80`) требует,
  чтобы обёртка с изменившимся СОДЕРЖИМЫМ приезжала НОВОЙ (строка обёрнута в
  `memo` и сравнивает `item` по ссылке).

Совместимы, потому что `useChatList` уже сохраняет ссылку `Chat` у строк, чьи
значения не менялись. Решение — ref-кэш обёрток по `chatId`: обёртка
переиспользуется, пока `value === value` предыдущей, и пересоздаётся ровно при
смене содержимого. Плюс стабильность самого МАССИВА (первое требование того же
контракта): пересчёт, не изменивший список поэлементно, возвращает ПРЕЖНИЙ
массив — иначе правка в чате другой папки заставляла бы `useShouldAnimate`
пересчитываться на пустом месте.

**Отступление от интерфейса из брифа: поле `index` у элемента убрано.**
`items` теперь `{id, value}[]` — форма `DeferredSortedVirtualListItem` ядра.
Причины: (1) ядро от поля уже отказалось с той же мотивацией
(`DeferredSortedVirtualList.tsx:47-50` — в tweb `index` читает `sortedItems`,
который мы не портируем), (2) единственный потребитель (`ChatList.tsx:203`)
его не читает, (3) держать в обёртке копию `chatsStore.dialogIndexById`
означало бы пересоздавать её на каждый `reindex` владельца — а он приходит на
любую правку `pinnedOrders`/`drafts` со ЗНАЧЕНИЯМИ диалогов неизменными, то
есть ровно ломать стабильность ссылок, ради которой кэш и заведён. Курсору
догрузки индексы по-прежнему нужны, и он берёт их у зеркала напрямую
(`fetchPage` → `useChatsStore.getState().dialogIndexById`), а не из `items`.

**Тесты** (второй — по результату, а не по реализации):

- «чат появился НАД видимой областью — сдвиг компенсирован, анимации нет»:
  реальная связка `useDialogListSource` + `useShouldAnimate` на 10 диалогах,
  `scrollAmount = 5*72`, `hostHeight = 144`; после `upsert` нового чата сверху
  ожидается `onScrollShift(-72)` и `shouldAnimate === false`;
- «изменилась одна строка — её обёртка новая, у остальных ТЕ ЖЕ ссылки» —
  обе половины контракта ядра в одном тесте;
- «правка вне папки пересчитывает items, но ссылка на массив прежняя»;
- «пересчёт, ничего не изменивший в списке, отдаёт ТОТ ЖЕ массив».
...
| 21 | **Task 7:** снято переиспользование обёртки из кэша (поведение до правки) | 2 теста: `expected {id:1,…} to be {id:1,…} // Object.is`, `expected [] to deeply equal [ -72 ]` |
| 22 | **Task 7:** снято переиспользование ПРЕЖНЕГО массива | `expected [ Array(1) ] to be [ Array(1) ] // Object.is` |
...
- **`items` больше не несёт `index`** — элемент это `{id, value}` (см. раздел
  про находку Task 7 выше). Нужен индекс порядка — он в
  `chatsStore.dialogIndexById`.
