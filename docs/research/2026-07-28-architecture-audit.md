...
Бэкенд не трогаем. Все находки привязаны к `file:line` на момент написания._
...
### P0-1. Реакции/опросы/read теряются при открытии чата из кэша
...
### P1-7. Пиры не в реактивном сторе
...
### P2-2. Прямые записи в доменные сторы из 20 компонентов
...
1. **SSOT сообщений** — `messagesById: Map<mid,Message>` + окна/пины/треды как списки id. Одним ходом
   закрывает P0-1 (реакции из кэша), сильно упрощает P0-2/P0-4, убирает тройную логику и
   O(окна×длина)-патчи. Самый дорогой, но с наибольшим каскадом.
2. **pts на live-пути + gap-детектор** (P0-2, P0-4) — чинит дубли уведомлений/звука/unread. Частично
   можно до SSOT.
3. **Диспетчер апдейтов по тегу** (P0-3, `worker.ts:93`) — дёшево, снимает класс молчаливых багов
...
7. Дальше по вкусу: `peersStore` (P1-7), чистка `storeProjection` (P1-5), дробление `messagesManager`
   (P1-6), `useSettings(selector)` (P2-3), структура папок (P2-4).
