...
| **P0-2** дубли уведомлений/звука/unread на reconnect | ✅ Сделано | PR #72: `deliveredSeq` персистится (переживает рестарт); реакция-×2 — pts-дедуп |
| **P0-3** диспетчер апдейтов по тегу (не по полю) | ✅ Сделано | PR #72: `/sync` тегирует `{t,pts,d}`, воркер диспетчит по `t` |
| **P0-4** детектор разрывов (gap) | 🟡 Частично | PR #72: per-chat seq-gap → точечный catch-up (сообщения); полный кросс-тип gap — нет |
...
### P0-2. Дубли уведомлений/звука/непрочитанных при каждом reconnect — ✅ СДЕЛАНО (PR #72)
...
### P0-3. Диспетчер апдейтов по наличию поля — молчаливый мисроутинг — ✅ СДЕЛАНО (PR #72)
...
### P0-4. Нет детектора разрывов (gap) — 🟡 ЧАСТИЧНО (PR #72: gap по seq для сообщений)
