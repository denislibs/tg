...
| P6 | ~~5 сторов вне `stores/`~~ — оказались feature-модулями, не переносим | `settings`, `i18n`, `pip`, `pwa`, `webapp` | закрыто (PR-5) |
| P7 | ~~Коллизия имён `superMessagePort`~~ → `lib/twebMessagePort` | `rpc/` vs `lib/` | сделано (PR-6) |
| P8 | ~~Мёртвый `chatsCache`~~ — оказался живым; вводящее в заблуждение имя | `stores/chatsCache.ts` → `dialogsPersist.ts` | сделано (PR-7) |
| P9 | 4 свалки утилит без правила «что куда» | `helpers/`, `core/*.ts`, `lib/`, `shared/lib/` | алиасы+правило сделаны (PR-8); массовый перенос отложен |
...
### PR-5 — ~~Собрать «дикие» сторы в `stores/`~~ → НЕ переносим *(посылка неверна)*

**Проверка кода отменила пункт.** Все 5 «диких» `create()` — не заблудившиеся сторы, а
**feature-модули**, где маленький стор колокейтед со своей логикой/провайдером:
`pip.ts` (+`enterPip`/`pipSupported`), `pwa.ts` (+`initPwaInstall`), `webapp.ts`
(+`openWebApp`/`webAppTheme`), `settings.tsx` (+`useSettings`/`formatTime`, 75 импортёров),
`i18n/index.tsx` (+`t`/`loadLang`/`useT`, 120 импортёров). Вынос только `create()` в `stores/`
**расщепил бы связный модуль** (стор отдельно от его логики/DOM) и сменил 5–120 импортов
каждый — ухудшение когезии ради формальной конвенции.

**Решение:** не переносить. В `CLAUDE.md` зафиксировано: отдельно стоящие сущностные сторы —
в `stores/`; мелкий стор, колокейтед с фичей, остаётся рядом с ней (это норма).

---

### PR-6 — Снять коллизию имён `superMessagePort` *(сделано → `lib/twebMessagePort`)*
...
### PR-8 — Схлопнуть свалки утилит *(алиасы+правило СДЕЛАНЫ; массовый перенос отложен)*

> **Статус:** алиасы `@core @stores @shared @rpc` добавлены (tsconfig+vite), `vitest.config`
> синхронизирован (починил 5 не-грузившихся тест-файлов), правило раскладки зафиксировано в
> `CLAUDE.md`. **Массовый перенос файлов между 4 корзинами НЕ делался** — это десятки git-mv
> + правки импортов, которые лучше катить отдельными мелкими пачками по подтеме (высокий риск
> merge-конфликтов, `helpers/`/`core/` трогают многие ветки). Ниже — исходный план на перенос.
...
**Что сделать (перенос — отложено).**
- Зафиксировать правило (в `web-client/CLAUDE.md`): *(сделано)*
