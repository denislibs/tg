...
СДЕЛАНО миграцией `0126`: у истории появился номер внутри автора (`stories.seq`
плюс счётчик `users.last_story_seq` — зеркало `chats.last_seq`), горизонт живёт
в `story_read (viewer, author, max_read_id)`, а признак `viewed` исчез и с
провода, и из надстроек схемы. «Непрочитанная» выводит клиент сравнением
`story.id > maxReadId` — ровно как непрочитанность сообщения по
`read_inbox_max_id`.
...
- **Горизонт потянул за собой АДРЕСАЦИЮ.** Номер историй стал пер-авторским,
  и сам по себе он историю больше не адресует: у разных авторов номера
  совпадают. Все ручки действия перешли на пару «пир автора + номер»
  (`/stories/{peerID}/{storySeq}/…`) — тем же ходом, каким сообщение перешло на
  `/chats/{peerID}/messages/{msgSeq}`. Внутренний ключ строки остался
  внутренним: связанные таблицы (просмотры, реакции, allow-лист) им и
  пользуются, а наружу его не выходит.
- **Ссылка репоста тоже адресует парой.** `fwd_from_story_id` хранил глобальный
  ключ источника; миграция перевела его в номер внутри автора, который лежит
  рядом (`fwd_from_author_id`) — то есть `storyFwdHeader{from, story_id}`
  собирается без единого лишнего запроса.
- **«Просмотрел» и «прочитал» оказались разными фактами.** `story_views`
  отвечает «кто посмотрел» (`getStoryViewsList`), `story_read` двигает горизонт
  (`readStories`); у оригинала это два метода. Наша ручка `view` делает оба, и
  это названное упрощение: интерфейс всё равно зовёт их вместе. Stealth-режим
  гасит ОБА — иначе «невидимый просмотр» выдал бы себя погасшим кольцом на
  другом устройстве.
- **Прочтение начало рассылаться.** Кадра у него не было вовсе, и вторая вкладка
  о нём не узнавала; теперь уезжает `updateReadStories{peer, max_id}` — на
  другие устройства ЗРИТЕЛЯ.
