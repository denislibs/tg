...
- **Вид медиа истории пришлось выводить.** Столбца `type` у истории нет, и
  ...
  один раз на сервере.
- **Кадр `story_new` построить историю не мог.** Он нёс плоские поля, а у
  `storyItem` обязательна ступень медиа, — и на шаге C витрине пришлось
  перечитывать ленту целиком, иначе в зеркало попала бы история с `media: null`.
  Шаг D это снял: кадр стал `updateStory` и несёт историю ЦЕЛИКОМ, поэтому
  применяется точечно; полный рефетч остался ровно для НОВОГО автора, чьей
  карточки в зеркале ещё нет.
- **Реакция оказалась ДВУМЯ фактами, а не одним.** Прежний `story_reaction`
  нёс общий счётчик вместе с `user_id`, и получатель решал «моё ли это»
  сравнением с собой. Теперь автору уезжает `updateStory` со свежей историей
  (агрегат живёт внутри неё), а самому зрителю — `updateSentStoryReaction` с его
  личным выбором: та же граница, что у `sent_reaction` внутри истории.
- **Кадру понадобилось читать историю.** `updateStory` несёт её целиком, а
  сервис после публикации/реакции держал в руках только номер — появился
  `StoryRepo.ByID`. История читается глазами АВТОРА, а пер-зрительские части
  (`viewed`, `sent_reaction`, `privacy`) из общего тела кадра вычищаются: тело
  одно на всех получателей, и это та же ловушка, что уже ловилась у `pFlags.out`
  и у `unread`.
