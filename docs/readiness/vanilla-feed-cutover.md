...
**это шире оригинала**, где `SERVICE_AS_REGULAR` (`bubbles.ts:275-279`) содержит
ТОЛЬКО звонок; подарок в тот список не входит и должен оставаться служебным
(разбор — в долге ниже).
...
**Долг (новый):** `messageActionStarGift` маршрутизируется в `kind: 'gift'`
(`web-client/src/core/messages/messageKind.ts:99-101`) и рисуется обычным
баблом со временем — тем же путём, что и звонок (`kind: 'call'`), — тогда как
в оригинале только звонок относится к `SERVICE_AS_REGULAR`
(tweb `bubbles.ts:275-279`); подарок остаётся служебным экшен-баблом. Пока у
нас нет своего бабла для звёздного подарка (пункт выше), это расширение
`SERVICE_AS_REGULAR` — единственный способ не показать пустую пилюлю, но по
факту это подгонка под текущий пробел, а не порт: правильный порт — свой
бабл `messageActionStarGift` (медиа/анимация подарка), а `getMessageKind`
для него должен вернуть `'service'`, как в оригинале.

**Долг (новый):** `message_ack` не приходит на ВТОРОЕ и последующие сообщения,
отправленные в одной сессии, — первое подтверждается, дальше кадр молчит.
К вёрстке ленты отношения не имеет и этой веткой не внесён (её диф ws-путь не
трогает), но записан здесь потому, что маскирует проверки: исходящий бабл не
доходит до состояния «отправлено», и живьём не проверить ни его сторону
(`is-out`), ни хвост на ней, ни статус времени. Воспроизводилось на стенде
дважды, в том числе после сброса Service Worker и перезагрузки.
