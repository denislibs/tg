...
**Счётчики поста едут ПАРОЙ и только у поста — зато всегда.** `views` и
`forwards` делят один бит (`views:flags.10?int forwards:flags.10?int`, предикат
`message`), поэтому у сообщения канала есть оба параметра либо нет ни одного;
в личке и в группе их нет никогда. У поста они стоят **с первой публикации**,
включая ещё не просмотренный: клиент отличает пост от прочего именно наличием
`views` (tweb `bubbles.ts:7672`, `messageRender.ts:273`), и отсутствие параметра
значило бы «наблюдать за просмотрами не надо». По той же причине `views` у
поста не бывает нулевым: минимум — единица, как у своего ещё не отправленного
поста в оригинале (`appMessagesManager.ts:2930`, `views: isBroadcast && 1`).
