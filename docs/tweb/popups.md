...
| `src/helpers/positionMenu.ts` | порт `positionMenu` (у точки события) + `positionMenuTrigger` (от кнопки; потребитель — vanilla-медиавьювер, `mediaViewer/appMediaViewer.ts:350`). `positionFloatingMenu`/`canMenuFitDirection`/`getMenuLeftPositionForDirection` не портированы — их потребители (`floatingButtonMenu`, `createSubmenuTrigger`) отсутствуют |
| `src/helpers/contextMenuController.ts` | порт синглтона `contextMenuController` (1:1) — `openBtnMenu`/`close`/`addAdditionalMenu`/`closeMenusByLevel`, mousemove-слежка 100/40 px |
| `src/helpers/overlayClickHandler.ts` | порт базы `OverlayClickHandler`; `getOverlayRoot()` → `document.body`, `appNavigationController` → `pushEsc` + `pushLayer` |
| `src/helpers/overlayCounter.ts` | порт `overlayCounter` (1:1); пока читается только `isDarkOverlayActive` (класс `night` у меню) |
| `src/helpers/dom/attachContextMenuListener.ts` | порт (1:1): правый клик / long-press 400 мс + `cancelContextMenuOpening` |
| `src/components/buttonMenu.ts` | порт `ButtonMenuItem`/`ButtonMenuSync`/`ButtonMenu` (без ripple, checkbox/radio, iconDoc, avatarInfo, langPack — см. шапку файла) |
...
2. **`overlayCounter` портирован, но никто его не инкрементирует.** Класс есть (`helpers/overlayCounter.ts`, 1:1), однако ни один попап/медиавьювер ещё не ставит `isOverlayActive`/`isDarkOverlayActive`, поэтому анимации под открытым попапом по-прежнему не глушатся, а общего признака «есть открытый оверлей» фактически нет (в `hotkeys.ts:60–62` вместо этого эвристика `setTimeout` + `defaultPrevented`). Читатель пока один — `contextMenuController.shouldApplyNight`.
...
6. **`contextMenuController` + `positionMenu` портированы, но живут только в ванильном слое.** Инфраструктура меню есть (`helpers/contextMenuController.ts`, `helpers/overlayClickHandler.ts`, `helpers/dom/attachContextMenuListener.ts`, `components/buttonMenu.ts`, `positionMenu`) — на ней собирается контекстное меню ленты. React-меню (`shared/ui/Menu`) на неё ещё НЕ переведены: каждый вызывающий сам меряет клик и решает флип по захардкоженным габаритам (`MW=256, MH=440` в `useMessageActions.tsx:136`; `MW=220, MH=320` в `ChatListItem.tsx:85`), закрытие по клику вне — собственный `.backdrop` у каждого меню вместо документ-контроллера. Не портированы `ButtonMenuToggle`, `createSubmenuTrigger`, `floatingButtonMenu`, `createContextMenu`.
