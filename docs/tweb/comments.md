...
| Тема | tweb | у нас |
|---|---|---|
| Футер поста | `replies-element` внутри `.bubble-content`, порядок `avatars → text → icon → rp` | `CommentsBar.tsx` — те же классы, порядок совпадает; нет `beside`-варианта, нет `is-unread`-точки; при `count === 0` показываем «Comments», а tweb — «Leave a comment» |
| Данные счётчика | `message.replies` + `replies_updated` + `getDiscussionMessage` | REST `/channels/{id}/comment_counts` + `useChannelExtras` (батч по постам) |
| Открытие треда | `setInnerPeer({peerId: ГРУППА, type: Discussion, threadId: mid зеркала В ГРУППЕ})` | `openCommentsThread({chatId: discussionChatId, rootMsgId: postId})` — пир группы совпадает, но корень треда = **id поста в канале**, а не id зеркала в группе |
