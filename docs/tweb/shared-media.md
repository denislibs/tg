...
| `members` | `GET /chats/{peerID}/members` — `router.go:317` → `group_handler.go:639`, репо `grouprepo.go:451` | участники + карточки users | **портируется**; пагинация `offset`/`limit` (дефолт 200) |
