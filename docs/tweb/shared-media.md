...
Панель профиля работает на портированном классе `components/appSearchSuper.ts`
(волна 3, задачи 5-13): его в панель ставит хук-шов
`core/hooks/useSearchSuper.ts` (роль tweb `AppSharedMediaTab` +
`sharedMedia.tsx` — скроллер вкладки, список вкладок, живые апдейты,
`setQuery → cleanupHTML → load(true)` на смену пира), а Solid-карточка
`components/peerProfile.solid.tsx` получает `searchSuper.container` пропом
`searchSuperContainer` и вставляет его последним ребёнком `.profile-content`
(1:1 `sharedMedia.tsx:166`). Панель `UserInfoPanel.tsx` исполняет только
контракт шапки (`sharedMedia.tsx:484-517`: `onAdditionalScroll` скроллера меряет
`searchSuper.nav`/`container`, `setIsSharedMedia` toggl-ит `is-full-viewport` и
зовёт `cleanScrollPositions()`, подзаголовок — из `onLengthChange`).
React-компонент `userInfo/SharedMedia.tsx` (815) **удалён** задачей 13 вместе с
`TAB_GAP`/`stickyTop`, порталом и загрузкой участников в `useGroupInfo`.

Геометрия класса требует, чтобы карусель была ПЕРВЫМ ребёнком `.profile-content`
(tweb `:196`): позиция «свежей» вкладки меряется от верха `container.parentElement`
(`appSearchSuper.ts`, порт `:653-661`). Поэтому задача 13 заодно закрыла бэклог
`profile-avatar-inside-solid-root.md` — `PeerProfileAvatars.container` едет в
Solid-корень пропом `avatarsContainer` тем же контрактом узла-пропа; на стенде до
этого «свежая» вкладка вставала на высоту карусели выше ряда, режим шаред-медиа
сбрасывался и память позиций терялась. Инвайт-ссылка приватной группы/канала —
тоже внутри корня: ветка `exported_invite` строки `Link` (`:999-1004`) портирована
в `peerProfile.solid.tsx` (проп `exportedInviteUrl`), React-сиблинг после корня
попадал под absolute-узел `.search-super`.

Решение по скроллеру (расхождение 7 в шапке класса): скроллер профиля ОДИН на
шапку и подсистему, его создаёт и роняет хозяин (хук, как `sliderTab.ts:66/109`),
а `AppSearchSuper.destroy()` его не роняет — снимает только свой
`onScrolledBottom`. Пин — `appSearchSuper.seam.test.ts` (настоящий `scroll`
после `destroy()` доходит до колбэка хозяина); шов — `useSearchSuper.test.tsx`
(узел один на панель, переезжает в новый корень, после размонтирования следов
нет); форма панели — текстовые пины `UserInfoPanel.shell.test.ts`.

## 2.1 Статус по подсистемам

| Подсистема оригинала | Статус | Наш код | Расхождение |
|---|---|---|---|
| Вкладки Media/Files/Links/Music/Voice | **есть, в ядре класса** | Media: `components/appSearchSuper.ts` — `processPhotoVideoFilter` (порт `:874-938`) + `onMediaClick` (порт `:716-767`), пины — `appSearchSuper.media.test.ts`; Files/Music/Voice — `processDocumentFilter`, Links — `processUrlFilter` (строки ниже) | Media портирован дословно: `div.grid-item` в `.search-super-content-media-grid`, `choosePhotoSize(media, 200, 200)`, `wrapPhoto`/`wrapVideo({onlyPreview, withoutPreloader, noPlayButton, photoSize})`, крышка `wrapMediaSpoiler` по `pFlags.spoiler`; первый клик по крышке снимает её, иначе открывается вьювер, и цели листания — `.grid-item` ЭТОЙ вкладки в её порядке (`newest-first`, `reverse: false`), за их пределами — `loadMoreMedia` той же ручкой (роль `setSearchContext`). Расхождения 13-17 в шапке файла: нет «чувствительного контента», `getMessageByPeer` заменён кэшем вкладки, `multiply` у крышки не передаётся. Документы/ссылки — строки ниже |
...
| Вкладка «Участники» | **есть в ядре класса** (задача 11) | `components/appSearchSuper.ts::loadMembers` (порт `:1525-1758`), пины — `appSearchSuper.members.test.ts` | список грузится САМ, пагинация 50 → 200 (`groups.channelParticipants`), клик → `openPeer`, меню участника; с задачи 13 хостом является панель через `useSearchSuper` (`openPeer` → `onOpenPeer`, `openUserPermissions` → `RightsEditor`), загрузка участников из `useGroupInfo` снята (пин `useGroupInfo.test.tsx`). Расхождения 31-35 в шапке класса: портирована только ветка канала; живые обновления — по `rt:chat_update` перечитыванием окна (кадра `updateChannelParticipant` нет, § 3); `slider` → колбэки `openPeer`/`openUserPermissions` |
| Вкладка «Подарки» | **есть в ядре класса** (волна 3, задача 12) | `appSearchSuper.ts::loadGifts` (порт `:2130-2179`) + `setPinnedGifts` (`:2579-2610`) + ветка `gifts` в `canLoadMediaTab` (`:2363-2365`) + `updateContainerHidden` (`:2520-2529`); витрина — Solid `components/stargifts/profileList.solid.tsx` (порт `stargifts/profileList.tsx`) поверх `profileStore.solid.ts` (порт `profileStore.ts`) и `stargiftsGrid.solid.tsx` (порт `stargiftsGrid.tsx`, вид `profile`) + `stargiftBadge.solid.tsx`; монтаж — мост `mountSolid` в `mediaTab.itemsTab`; пины — `appSearchSuper.giftsSaved.test.ts`, `stargifts/profileList.solid.test.tsx`. React-копия снесена задачей 13 | Расхождения 36-39 в шапке класса. Портировано ровно то, что покрыто ручкой `GET /users/{id}/gifts` и моделью: сетка плиток, метка скрытого (`unsaved`), бейдж «1 of N», даритель/аноним, пустое состояние `StarGiftCollectionsEmptyOther`, прелоадер, счётчик → строка ряда/`is-single`/градиент, топ-3 в имени вкладки. **Нет предмета** (блокеры, не пропуски): коллекции (`ChipTabs`, `handleSwipe`, попапы) и фильтры/сортировка — ручка без параметров; стикер подарка — в модели unicode-символ (`gift.emoji`), им и рисуются плитка и `setPinnedGifts`; `pinned_to_top`/коллекционные (`starGiftUnique`) — полей нет; контекстное меню плитки — `createContextMenu` не портирован; клик по плитке — `PopupStarGiftInfo` не портирован (`onClick` грида свободен); живые апдейты — событий подарков в `rootScope` нет |
| Вкладка «Чаты» (savedDialogs) | **есть в ядре класса** (волна 3, задача 12) | `appSearchSuper.ts::loadSavedDialogs` (порт `:1890-1941`); список — Solid `components/sidebarRight/savedDialogsTab.solid.tsx` поверх `components/verticalVirtualList.solid.tsx` (порт `verticalVirtualList.tsx` 1:1, + `helpers/solid/{createAnimatedValue,useElementSize,requestRAF}.ts`); строка — разметка `DialogElement` (`addListDialog`, `avatarSize: 'bigger'`, `row-big`), аватар — `avatarNew`; клик — `core/navigation/openPeer.ts` (общий с React-хуком `useNavigationActions`); пины — `appSearchSuper.giftsSaved.test.ts`, `sidebarRight/savedDialogsTab.solid.test.tsx` (все сценарии прежнего `SharedMedia.saved.test.tsx` перенесены, React-пины стабильности ссылок предмета не имеют). React-копия снесена задачей 13 | Расхождение 40 в шапке класса: набор одним RPC (`chats.savedDialogs`) — `side`/пагинация/скелетоны/волна раскрытия `deferredSortedVirtualList.tsx` не портированы, счётчик — длина набора; `openSavedDialogsInner`/`slider` не в опциях — окна сохранённого диалога у нас нет, клик открывает оригинальный чат пира; живых апдейтов (`dialogs_multiupdate`/`dialog_drop`) нет — событий нет; контекстное меню строки — `createContextMenu` не портирован. Пустой набор оставляет `ul` высотой 0 (как оригинал), вкладку прячет счётчик — React-заглушка «Nothing here yet.» не переносилась |
...
| Ряд вкладок (разметка) | **есть, в ядре класса** | `components/appSearchSuper.ts` (волна 3, задача 5); список вкладок правой колонки — `core/hooks/useSearchSuper.ts::SHARED_MEDIA_TABS` (8 из 12 `sharedMedia.tsx:604-648`, без `stories`/`saved`/`groups`/`similar` — задачи 16-19) | ядро строит ряд как оригинал (`appSearchSuper.ts:462-493`) и сверено с дампом `dom/dumps/07-right-sidebar.json:121-131`; React-копия снесена задачей 13 |
...
| Анимация перехода между вкладками | **есть, потребитель — ядро класса** | `components/transition.ts` — `TransitionSlider` с `slideTabs`; React-`TabSlide.tsx` остался у `ChatList`/`SearchView`, `useTransitionSlider.ts` — у заголовка панели | `slideTabs` (`transition.ts:45-95`) портирован 1:1 (волна 3, задача 3): вкладки не размонтируются, а сдвигаются на ±width, поэтому поддерево и `scrollTop` переживают переключение. Потребитель — `AppSearchSuper` через `horizontalMenu`; React-путь шаред-медиа с `keepMounted`-дефектом снесён задачей 13 |
...
| Пагинация | **есть, в ядре класса** | `appSearchSuper.ts::loadType` (порт `:2181-2360`), `scrollable.onScrolledBottom` — хук класса на скроллере хозяина | курсором `offset_id` (задача 1 — бэкенд), догрузка по низу скроллера, как в оригинале; React-`IntersectionObserver`-сентинел снесён задачей 13 |
| Кэш вкладок | **есть, 1:1** | `components/sharedMediaHistories.ts` (`getHistoryStorage(peerId)`), хук-шов отдаёт его классу в `setQuery` на каждую смену пира | модульный, переживает смену пира — возврат в профиль рисует вкладки без сети (пин `useSearchSuper.test.tsx`, мутация M5) |
| Живые апдейты | **есть, 1:1** | `sharedMediaHistories.ts::renderNewMessage`/`deleteDeletedMessages` (порт `sharedMedia.tsx:209-345`), подписка `subscribeSharedMediaLiveUpdates` — из хука-шва (`:596-602`) | точечный prepend/remove; сброс кэша по длине окна снесён вместе с React-панелью. Своё сообщение доезжает ПО ACK (`history_update` + `tempId`), временный бабл с дробным id пропускается — расхождение 4 в шапке файла (у оригинала подсистема слушает `history_multiappend`, только сохранённые); найдено на стенде задачи 13: узел с временным mid переживал `history_delete` |
...
| Ядро виртуального списка (Solid-форма) | **есть, 1:1** (волна 3, задача 12) | `components/verticalVirtualList.solid.tsx` (порт `verticalVirtualList.tsx`, 191) + `helpers/solid/createAnimatedValue.ts` (порт `helpers/solid/createAnimatedValue.ts`), `helpers/solid/useElementSize.ts` (порт `hooks/useElementSize.ts`), `helpers/solid/requestRAF.ts` | Исходная Solid-форма того же ядра, что раньше портировано под React (`components/virtual/VerticalVirtualList.tsx`): окно — `createSelector` + `<Show>` на строку, без троттлинга. Компенсатор сдвига `scrollTop -= amount` — в allow-list `core/scrollWriters.test.ts`. Надстройка `createDeferredSortedVirtualList` (дырки/скелетоны/reveal/shrink) в Solid не портирована — у единственного потребителя (вкладка «Чаты») нет пагинации |
...
Все пять ушли вместе с React-`SharedMedia.tsx` (задача 13, 2026-09-07):

1. **`stickyTop = TAB_GAP` = 8px перебивал портированный CSS** — снят вместе с
   `TAB_GAP`; липкий ряд стоит на `top: var(--super-offset)` (`_searchSuper.scss:24`).
   Пины: `useSearchSuper.test.tsx` («нет инлайнового top»), `UserInfoPanel.shell.test.ts`.
   Закрывает P1 из `docs/research/2026-08-08-tweb-deep-structural-audit.md`.
2. **Инвалидация кэша слишком широкая** — точечные апдейты `sharedMediaHistories.ts` (задача 6).
3. **Пять запросов вместо одного** на счётчики — один batch `searchCounters` (задача 10).
4. **Неактивная вкладка размонтировалась** — `slideTabs` класса держит DOM и `scrollTop` (задачи 3, 5).
5. **Инлайн-стиль пустого состояния** — `.content-empty` из `afterPerforming` (задача 6).

Стенд 2026-09-07 до переключения фиксировал ещё три симптома React-панели,
которых у класса нет по построению: рывок/отсутствие выезда при смене вкладки
(кадр вставлялся в DOM в одном кадре с transform — `TabSlide.tsx`), сетка медиа в
одну колонку (`div.grid` вместо `search-super-content-media-grid`) и строка
ссылки-приглашения под липким рядом (тот же инлайновый `stickyTop`).
...
|---|---|---|
| **Владение узлом между React и ванилью** | Узел `search-super` создавал React (`UserInfoPanel.tsx`) и рисовал в него портал; у оригинала `searchSuperContainer` это **и есть** `tab.searchSuper.container` (`sharedMedia.tsx:166`) | **Закрыто задачей 13**: Solid-карточка получает `searchSuper.container` из хука-шва `useSearchSuper`, снимает узел класс в `destroy()`. Пин на шов — `useSearchSuper.test.tsx` (после размонтирования узлов и живых async-веток класса нет) |
| **Двойная реализация** | Соблазн оставить React-`SharedMedia.tsx` «пока» рядом с классом. Ровно этим программа проваливается (DoD п. 14) | **Закрыто задачей 13**: переключение шва и удаление React-компонента — один коммит, `git grep userInfo/SharedMedia` пуст |
...
| **Ванильного чатлиста нет** | `SortedUserList` (`sortedUserList.ts:134`) опирается на `appDialogsManager.addDialogNew`, которого у нас нет | Закрыто задачей 11: узкий строитель строки `components/dialogRow.ts` |
...
| **Тесты на текущий React дадут ложный зелёный** | `SharedMedia.invalidate.test.tsx` пинил именно ту инвалидацию, которую надо было снести | Снесён с компонентом (задача 13); контракт оригинала держит `appSearchSuper.live.test.ts` |
