...
3. **Класс позиционирования строки навешивается layout-эффектом ПОСЛЕ КАЖДОГО
   коммита, а не однократно при создании узла.** В оригинале
   (`deferredSortedVirtualList.tsx:176`) строка — vanilla-узел, её классы правит
   точечный `classList.toggle`, поэтому поставить класс один раз достаточно. У
   нас строку рендерит React, и атрибутом `class` владеет он: первая же смена
   `className` потребителя (выделение чата, unread) перезаписала бы атрибут
   целиком и сняла бы класс — строка потеряла бы `position: absolute` и
   вывалилась из потока списка. Отсюда `useLayoutEffect` БЕЗ deps
