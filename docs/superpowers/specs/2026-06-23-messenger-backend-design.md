...
Токен устройства → хранится хэшем в `devices.token_hash` (источник правды — Postgres). Каждое
устройство = отдельная сессия (фундамент под мультиустройство/мульти-аккаунт и мобильные клиенты:
web/ios/android различаются полем `platform`).

### 9a. Управление сессиями + Redis-кэш токенов (Plan C)
Реализуется вместе с введением Redis в Plan C.

**Эндпоинты (под auth):**
```
GET    /sessions            → список устройств юзера [{id, name, platform, last_active, current}]
DELETE /sessions/{id}       → отозвать конкретную сессию (удалить device + сбросить кэш)
POST   /auth/logout         → отозвать текущую сессию (по Bearer-токену)
```
- `current: true` помечает устройство, с которого пришёл запрос (по `token_hash`).
- Отзыв сессии: удалить строку `devices` + удалить ключ `session:{token_hash}` из Redis. После
  этого WS-соединение этого устройства (Plan C) принудительно закрывается, токен невалиден.

**Redis-кэш токенов (`session:{token_hash}` → {user_id, device_id}, TTL):**
- Middleware сначала смотрит Redis; промах → Postgres + заполнение кэша. Снимает нагрузку с БД на
  каждый запрос/кадр WS.
- Инвалидация: при `logout`/revoke удаляем ключ; иначе истекает по TTL (`last_active` обновляем
  лениво/батчем, чтобы не писать в БД на каждый запрос).
...
Phase 0 (ядро) реализуется четырьмя планами:
- **Plan A** — Foundation + Auth ✅
- **Plan B** — Chats + Messages + History + Sync (REST) ✅
- **Plan C** — Realtime (Redis, WS-хаб, fan-out, presence, typing, live read/reactions)
  **+ управление сессиями (§9a) + Redis-кэш токенов** (Redis вводится здесь)
- **Plan D** — Медиа (MinIO) + Web Push

| # | Подсистема | Зависит от |
|---|-----------|-----------|
| 0 | Ядро: auth + realtime + личные чаты + session management | — |
