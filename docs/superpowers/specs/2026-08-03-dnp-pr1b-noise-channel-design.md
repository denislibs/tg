...
**Мандат корректности (уточнён на реализации PR-1b-i):** основной якорь — **детерминированный
Go↔JS interop-тест** против фикстуры, сгенерированной `flynn/noise` (каноничная референс-реализация,
сама вектор-валидированная в апстриме): JS-initiator байт-в-байт воспроизводит `msg1`/`msg2`/
транспортный ct эталона (см. §7). Плюс якорь примитивов: BLAKE2s-256 KAT и HKDF-конструкция в
юнит-тестах. Это принято как достаточное. Прогон **официальных cacophony/noise-c NK-векторов** —
опциональное будущее усиление (не блокирует). Interop-гейт обязателен — без него не мёржить.
