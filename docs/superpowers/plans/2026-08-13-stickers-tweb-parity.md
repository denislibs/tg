...
## Проверка в браузере (2026-08-13)

Сборка ветки поднята отдельным nginx-контейнером на `https://localhost:38444`
(тот же образ и конфиг, что у стенда `msgrverify`, том — `client-build` ворктри;
стенд пользователя на :38443 не тронут, контейнер после проверки удалён).

Проверено на живом стенде:

| Что | Результат |
|---|---|
| `player.offscreen` у стикера | `'canvas'`, `offscreenViaCompositor: true` |
| canvas в DOM | `data-offscreen="1"` — передан в воркер (`transferControlToOffscreen`) |
| Пиксели на экране | да (пробная анимация крутится; панель стикеров — все 8 стикеров набора) |
| `presentSticker` (awaitable RPC в compositor-воркер) | отвечает за ~0 мс |
| Панель стикеров целиком | 10 offscreen-канвасов, все нарисованы |

**Найдено попутно:** `ensurePresented()` в СКРЫТОЙ вкладке не резолвится —
не из-за RPC (он отвечает), а из-за `doubleRaf` (`PRESENT_PAINT_WAITS`): `rAF`
в фоновой вкладке не тикает. Поведение унаследовано от tweb 1:1 и безвредно
(скрытая вкладка не рисует; при показе rAF возобновляется и нижний слой
снимается), но знать про него надо: в скрытой вкладке превью висит под медиа.

Не проверено в браузере: стикер в БАББЛЕ (в тестовой БД нет ни одного сообщения
со стикером) и слой `thumb` с бэка на webp-стикере (все стикеры стенда —
lottie, у них `blur_preview` пуст by design).
