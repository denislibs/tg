...
- После каждой задачи до коммита: `npm test` и `npm run typecheck` — **зелёные целиком**, в отчёт вставлять реальный хвост вывода.
- Линт: `npm run lint` по репозиторию **грязный и до нас** (~2378 ошибок и сотни предупреждений в чужих файлах — `src/core/net/dnp/*`, `src/components/webapp/*` и т.д.). Чинить этот базлайн в этой ветке НЕ надо. Критерий — **свои файлы чисты**: `npx oxlint --type-aware <твои файлы>` не печатает ничего. Его вывод (пустой) и приводить в отчёте.
