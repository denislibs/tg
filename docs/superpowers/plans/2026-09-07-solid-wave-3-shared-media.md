...
- [ ] **Шаг 1: прочитать** `messagesrepo.go:449-487` целиком и `router.go:261`.
- [ ] **Шаг 2: падающий тест** на репозиторий: `offset_id = seq` последнего
...
- [ ] **Шаг 4: реализовать.** `GET /chats/{peerID}/media` принимает `offset_id`
  (0/отсутствует = с начала); `WHERE m.seq < $offset_id` вместо `OFFSET`.
  Старый `offset` **удалить**, а не оставить второй веткой (мёртвый код).
- [ ] **Шаг 5: индекс.** Миграция с составным индексом под фильтрацию по типу;
  в теле коммита — `EXPLAIN` до и после на чате с ≥50k сообщений.
- [ ] **Шаг 6: клиент.** `core/managers/messagesManager.ts:700-703` —
  `mediaHistory(peerId, filter, offsetId, limit)`. Текущий единственный
  потребитель (`SharedMedia.tsx:157,193`) переводится на новую форму.

**Готово когда:** ручка листает по `offset_id`; тест на «вставка сверху не рвёт
вторую страницу» краснеет на возврате к `OFFSET`; `offset` в коде не осталось.
...
- [ ] **Шаг 1: падающий тест** на юзкейс: запрос по списку фильтров возвращает
  число по каждому одним ответом; неизвестный фильтр — ноль, а не ошибка.
- [ ] **Шаг 2: убедиться, что тест падает.**
- [ ] **Шаг 3: реализовать** `GET /chats/{peerID}/search_counters?filters=media,files,links,music,voice`
...
- [ ] **Шаг 4: клиент** — `managers.messages.searchCounters(peerId, filters)`.
...
- [ ] **Шаг 1: прочитать** `tweb/src/components/transition.ts` целиком (383) и
  наш `core/dom/navigationTransition.ts` целиком.
- [ ] **Шаг 2: падающий тест**: при переходе 0→1 уходящий получает `from`,
  приходящий — `active to`, контейнер — `animating`; при 1→0 добавляется
  `backwards`; по `transitionend` все временные классы сняты; страховочный
  таймер (`tweb:350`, `transitionTime + 100`) снимает их и без события.
- [ ] **Шаг 3: убедиться, что тест падает.**
- [ ] **Шаг 4: реализовать**, дословно. Сохранить `selectTab.prevId()/.getFrom()/.setFrom()`
  (`tweb:376-378`) — они нужны правой колонке.
- [ ] **Шаг 5: снести** `core/dom/navigationTransition.ts`, потребителей перевести
...
- [ ] **Шаг 1: прочитать оригинал целиком.**
- [ ] **Шаг 2: падающий тест**: клик по вкладке зовёт `onClick(id, content, animate)`;
...
- [ ] **Шаг 1: прочитать** `appSearchSuper.ts:1-820` и `:2700-2843` целиком.
- [ ] **Шаг 2: падающий тест на DOM**: конструктор строит
...
- [ ] **Шаг 3: падающий тест на память скролла** (главный пин задачи):
...
- [ ] **Шаг 4: убедиться, что оба теста падают.**
- [ ] **Шаг 5: реализовать** конструктор, `selectTab`-коллбэки, `scrollToStart`,
  `sliding`, свайп.
- [ ] **Шаг 6: тест на очистку**: `cleanupHTML()` вычищает `itemsTab`, возвращает
...
- [ ] **Шаг 7: реализовать** очистку и `setQuery` (пересборка `searchContext`,
...
- [ ] **Шаг 1: прочитать** `appSearchSuper.ts:1096-1283`, `:2181-2360`, `:2531-2577`
  и `sharedMedia.tsx:28-345` целиком.
- [ ] **Шаг 2: падающий тест на кэш**: первая загрузка вкладки идёт в сеть;
...
- [ ] **Шаг 3: падающий тест на пагинацию**: вторая страница запрашивается с
  ...
  (дедупликация `:2189`).
- [ ] **Шаг 4: падающий тест на живые апдейты** (пин против нашей нынешней
...
- [ ] **Шаг 6: реализовать.**
- [ ] **Шаг 7:** `historiesStorage` — модульный, ключ `peerId → threadId → filter`,
...
`cleanup()` начнёт чистить `historyStorage`.
...
- [ ] **Шаг 1: прочитать** `:940-962` и наш `components/wrappers/document.ts` целиком.
- [ ] **Шаг 2: падающий тест**: файл даёт строку документа с именем/размером/датой;
...
- [ ] **Шаг 1: прочитать** `:964-1094`.
- [ ] **Шаг 2: падающий тест**: сообщение с `webpage` даёт строку с заголовком,
