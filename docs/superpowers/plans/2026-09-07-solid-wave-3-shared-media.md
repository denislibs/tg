...
- [ ] **Шаг 6: клиент.** `core/managers/messagesManager.ts:697-700` —
...
- [ ] **Шаг 7:** `historiesStorage` — модульный, ключ `peerId → threadId → filter`,
  **переживает смену пира** (`sharedMedia.tsx:33-36`). Тест: вернулись к тому же
  пиру — сети нет.
- [ ] **Шаг 8: второй потребитель.** Перевести `Chat.tsx::loadMoreMedia`
...
`cleanup()` начнёт чистить `historyStorage`; **листание в просмотрщике медиа
работает** — оба потребителя `mediaHistory` переведены, `offset` больше никем не
посылается.
