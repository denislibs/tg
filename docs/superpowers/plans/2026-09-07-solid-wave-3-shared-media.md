...
- [x] **Шаг 1: прочитать** `:2362-2529` и `:2611-2703`.
- [x] **Шаг 2: падающий тест**: счётчики берутся ОДНИМ запросом (задача 2);
...
- [x] **Шаг 3: убедиться, что тест падает.**
- [x] **Шаг 4: реализовать.** `canViewGroups`/`canViewSaved` возвращают `false`
...
- [x] **Шаг 1: прочитать** `tweb/src/components/sortedUserList.ts` (134),
  `appSearchSuper.ts:1525-1758`, `tweb/src/helpers/dom/createParticipantContextMenu.ts`.
- [x] **Шаг 2: падающий тест на строку**: разметка совпадает с
  `docs/tweb/dom/dumps/15-right-*.json` (заголовок, роль правым слотом, статус,
  аватар `row-media-abitbigger`).
- [x] **Шаг 3: падающий тест на вкладку**: список пагинируется (первая партия 50,
...
> Первая половина сделана в ветке `feat/search-super-members` (список грузится
> сам, `groups.channelParticipants`); вторая — снятие загрузки из `useGroupInfo`
> — правка React-стороны и уезжает в задачу 13 (пункт 6 ниже), чтобы не
> сталкиваться со сносом `SharedMedia.tsx`.
...
- [x] **Шаг 1: прочитать** `tweb/src/components/stargifts/profileList.tsx` (472),
  `appSearchSuper.ts:1890-1941`, `:2130-2179`, `:2579-2610`.
- [x] **Шаг 2: падающий тест**: витрина подарков рисует сетку и пустое состояние
...
- [x] **Шаг 3: убедиться, что тест падает.**
- [x] **Шаг 4: реализовать.** Существующий виртуальный список «Избранного»
...
6. **Снять загрузку участников из `useGroupInfo`** (перенесено из «Готово
   когда» задачи 11): `core/hooks/useGroupInfo.ts:164-181` (эффект
   `managers.groups.members` → `setRealMembers`), `:185-197` (`refreshMembers`
   и его вызовы `:204`, `:214-229`), поле `realMembers` в результате хука
   (`:65`, `:243`) и проп `members={…realMembers…}` в `UserInfoPanel.tsx:696`.
   Список грузит сам класс (`loadMembers`, `groups.channelParticipants`);
   плоский `groups.members` остаётся экранам редактирования группы. Хосту
   класса передать `openPeer` (= нынешний `onOpenPeer`) и
   `openUserPermissions` (= `setEditMember` → `RightsEditor`) — расхождение 33
   в шапке класса.
...
- [x] **Шаг 1: падающий тест на шов**: `searchSuperContainer`, полученный
...
- [x] **Шаг 2: падающий тест на владение** (DoD 5): после `destroy()` панели
  узлов класса в документе нет.
- [x] **Шаг 3: падающий тест на липкость**: у `.search-super-tabs-scrollable` нет
...
- [x] **Шаг 6: живая проверка на стенде** (DoD 10) — прощёлкать пункты 1-7 и 10-11
