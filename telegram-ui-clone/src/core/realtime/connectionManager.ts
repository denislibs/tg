...
export interface SendArgs { chatId: number; text: string; entities?: MessageEntity[] | null; clientMsgId: string; replyToId?: number | null; mediaId?: number | null; type?: string; groupedId?: string; geo?: { lat: number; lng: number }; contactUserId?: number; threadRootId?: number | null }
...
const FRAME_TYPES = ['new_message', 'edit_message', 'delete_message', 'pin_message', 'read', 'media_read', 'chat_removed', 'typing', 'presence', 'reaction', 'draft_update', 'dialog_pin', 'dialog_archive', 'poll_update', 'message_ack', 'message_error', 'pong', 'call_request', 'call_accept', 'call_decline', 'call_end', 'call_signal', 'group_call_update', 'group_call_signal']
...
export function newConnectionManager({ ws, getToken, onReady, onState, onFrame, outboxStore }: CMDeps) {
  ...
  function sendFrame(m: SendArgs) {
    ws.send('send_message', { chat_id: m.chatId, type: m.type ?? 'text', text: m.text, entities: m.entities ?? null, client_msg_id: m.clientMsgId, reply_to_id: m.replyToId ?? null, media_id: m.mediaId ?? null, grouped_id: m.groupedId ?? '', geo_lat: m.geo?.lat ?? null, geo_lng: m.geo?.lng ?? null, contact_user_id: m.contactUserId ?? null, thread_root_id: m.threadRootId ?? null })
  }
