...
Версию языка назначает снимок, а не база: уменьшаться ей нельзя (клиент, ушедший
вперёд, перестал бы принимать строки навсегда), а база сбрасывается вместе с
откатом миграций. Единственный источник переводов — файлы клиента
(`web-client/src/lang.ts`, `web-client/src/i18n/dict.*.ts`); после их правки
снимок пересобирается, и та же команда растит версию изменившихся языков:
