...
func TestWorker_SendsPrunes410AndAcks(t *testing.T) {
	q := &fakeQueue{}
	_ = q.Enqueue(context.Background(), Job{RecipientID: 7, ChatID: 3, PeerID: -3, MsgID: 100, Seq: 5, SenderID: 9, Text: "hi", Preview: true})
	...
	// В теле пуша — ключ пира, а не внутренний chat_id.
	if got["peer_id"].(float64) != -3 || got["msg_id"].(float64) != 100 || got["seq"].(float64) != 5 {
