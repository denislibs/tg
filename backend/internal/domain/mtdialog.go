...
type DialogReal struct {
	...
	// Draft — flags.1?DraftMessage: черновик ЭТОГО диалога.
	//
	// Место черновика — сам диалог, и это не перестановка полей: пока он жил
	// отдельным хранилищем, «когда чат последний раз трогали» приходилось
	// собирать из двух источников (дата сообщения и дата черновика лежали
	// врозь), а порядок списка зависит именно от неё. Ручка /drafts остаётся —
	// у оригинала `messages.getAllDrafts` тоже отдаёт КАДРЫ, которыми клиент
	// заполняет те же диалоги; двух хранилищ от этого не появляется.
	//
	// «Черновика нет» — ОТСУТСТВИЕ параметра (flags.1), а не пустой
	// конструктор: draftMessageEmpty значит «черновик СНЯЛИ», и это событие,
	// а не состояние строки.
	Draft DraftMessage `json:"draft,omitempty"`
