...
var OmittedWithoutSubject = map[string][]string{
	...
	// Номер приложения в реестре MTProto: реестра у нас нет вовсе, а адресовать
	// клиента нечем — он один. `region` мы не различаем: город и страна
	// приезжают из GeoIP одной строкой, и разложить её обратно не по чему.
	// Имя и версия клиента, ОС и дата входа отсюда УШЛИ — они производятся
	// (докблок Authorization в mtaccount.go).
	//
	// Отказа `FRESH_RESET_AUTHORISATION_FORBIDDEN` мы тоже не производим: у
	// оригинала он запрещает свежей сессии отзывать чужие первые сутки, а у нас
	// такого правила нет — не текста не хватает, а самого правила.
