...
//
// `base_lang_code` — язык, из которого берутся недостающие строки. Наши
// переводы НЕПОЛНЫЕ (у русского переведено 1292 ключа из 1302, у остальных
// четырёх — 686-687), так что база у них действительно английский, и поле
// говорит правду.
//
// Но правду оно говорит НИКОМУ: оригинал этого параметра не читает нигде —
// во всём tweb `base_lang_code` не встречается ни разу. Фолбэк там устроен
// иначе: локально вшитые `lang.ts`/`langSign.ts` подмешиваются ПОД серверный
// пакет (`langPack.ts:238-244`), а строки, которой нет ни там, ни там,
// `I18n.format` отдаёт СЫРЫМ КЛЮЧОМ (`:480`, `:484`).
//
// Поэтому здесь названо и то, чего НЕТ: подмешивания базы на сервере не
// реализовано — `getLangPack('ru')` отдаёт ровно 1292 строки русского, а не
// 1302 с английскими заплатками. Дыры не видно, потому что клиент везёт свой
// английский словарь и кладёт перевод поверх него — это и есть принятое
// решение (задачи 5 и 9: `lib/langPack.ts::applyServerLangPack` кладёт
// локальный английский ПОД серверный пакет, и слой этот несъёмный). Перестань
// клиент везти английский бандл — 10 ключей у русского и около 616 у каждого
// из остальных приехали бы на экран сырыми символическими ключами.
//
// Числа здесь — СНИМОК на момент волны i18n, а не сторожевое утверждение:
// пересчитываются они генератором снимка (`go run ./cmd/langpackgen` печатает
// строки по языкам), и сверять их с ним — дело того, кто правит словари.
//
...
type LangPackLanguageMeta struct {
	...
	// Position — место языка в выдаче `getLanguages`.
	//
	// Порядок задаёт СЕРВЕР, потому что клиент его не задаёт: список рисуется
	// перебором выдачи без сортировки (tweb `language.tsx:117`). Параметром
	// схемы позиция не едет — её у `langPackLanguage` нет; она выражена самим
	// порядком вектора, как и у оригинала.
