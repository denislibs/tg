...
// конструктора из пяти.
//
// ОГОВОРКА ОБ ЭТАЛОНЕ: у этой подсистемы его нет. tweb секретные чаты не
// реализует вовсе — `requestEncryption` и `encryptedChatWaiting` встречаются у
// него только в самой схеме (`src/lib/mtproto/schema.ts`) и ни в одной строке
// кода. Значит сверить ПОВЕДЕНИЕ не с чем, и опираться здесь можно лишь на
// набор полей конструкторов.
//
// Что из набора полей следует: `encryptedChatWaiting` отличается от
// `encryptedChatRequested` ровно отсутствием `g_a`, то есть у оригинала две
// стороны видят РАЗНЫЕ конструкторы — тот, кто ключ инициатора уже знает
// (сам инициатор), и тот, кому его прислали. У нас обе стороны видят
// `encryptedChatRequested`: витрина одна. `encryptedChatEmpty` (нет такого
// чата) не возникает — отсутствие чата у нас отказ, а не конструктор.
