...
func NewRouter(authUC *usecaseauth.Interactor, chatUC *usecasechat.Interactor, wsHandler http.Handler, mediaH *MediaHandler, mediaUC *usecasemedia.Interactor, pushH *PushHandler, storyH *StoryHandler, memberPresence PresenceQuery, contactsUC *usecasecontacts.Interactor, iceH *ICEHandler, notifyUC *usecasenotify.Interactor, foldersUC *usecasefolders.Interactor, pubH *PublicHandler, privacyUC *usecaseprivacy.Interactor, passkeyH *PasskeyHandler, stickersH *StickersHandler, ivH *IVHandler, reportUC *usecasereport.Interactor, statsUC *usecasestats.Interactor) http.Handler {
	...
	r.Group(func(pr chi.Router) {
		...
		pr.Post("/chats/{chatID}/messages/{msgID}/transcribe", ch.TranscribeMessage) // расшифровка голосового/кружка
		pr.Get("/chats/{chatID}/send_as", ch.SendAs)                                 // «личности отправителя» (send_as)
		...
		pr.Get("/chats/{chatID}/message_by_date", ch.MessageByDate)
		pr.Get("/chats/{chatID}/calendar", ch.Calendar)
		...
		pr.Patch("/chats/{chatID}/invite_links/{token}", gh.EditInvite)
		pr.Delete("/chats/{chatID}/invite_links/{token}", gh.DeleteInvite)
		pr.Delete("/chats/{chatID}/revoked_invite_links", gh.DeleteAllRevoked)
		pr.Get("/chats/{chatID}/invite_links/{token}/importers", gh.InviteImporters)
		...
		pr.Put("/channels/{chatID}/discussion", chh.LinkDiscussion)
		pr.Delete("/channels/{chatID}/discussion", chh.UnlinkDiscussion)
		pr.Get("/channels/{chatID}/discussion_candidates", chh.DiscussionCandidates)
		pr.Put("/channels/{chatID}/sign_messages", chh.SetSignatures)
		...
		pr.Get("/channels/{chatID}/similar", chh.Similar)
