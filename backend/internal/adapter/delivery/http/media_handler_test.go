...
func newMediaRouter(t *testing.T) (http.Handler, *pgxpool.Pool) {
	...
	mediaUC := usecasemedia.New(pgadapter.NewMediaRepo(pool), newFakeStorage(), nil)
	mediaH := NewMediaHandler(mediaUC, chatUC, authUC, "test-secret")
	return NewRouter(authUC, chatUC, nil, mediaH, mediaUC, nil, nil, nil, nil, NewICEHandler("", "test"), nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil), pool
}
