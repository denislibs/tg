...
- **База знаний по tweb — в [`docs/tweb/`](docs/tweb/README.md)** (индекс: подсистема →
  какой док читать). Перед портом/починкой UI сначала док, потом исходники tweb, потом код;
  порядок работы задаёт скилл `tweb-parity`. Что портировать и в каком порядке —
  [`docs/tweb/roadmap.md`](docs/tweb/roadmap.md).
