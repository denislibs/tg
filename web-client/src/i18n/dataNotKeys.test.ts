 ...
 *
 * Имена ЗАКРЫТЫ списком и все до одного значат «перевести»: `t`/`tArgs` — само
 * хранилище, `i18n`/`i18n_` — ядро. Прежняя редакция принимала ещё и `title(` — под
 * него подошёл бы ЛЮБОЙ будущий не-переводящий `title()` внутри вызова менеджера, и
 * пин на этом месте ослеп бы молча. Предмета у исключения больше нет: единственный
 * вызывающий (`core/hooks/useSidebarActions.ts`) звал так свой же локальный псевдоним
 * переводчика, и псевдоним переименован в `t`.
 */
const inTranslationCall = (src: string, index: number) =>
  /\b(?:t|tArgs|i18n|i18n_)\(\s*$/.test(src.slice(Math.max(0, index - 16), index))
 ...
 *
 * СКОБКИ ВНУТРИ СТРОКОВЫХ ЛИТЕРАЛОВ ПРОПУСКАЮТСЯ. Прежняя редакция считала баланс по
 * всем символам подряд, и любая `)` внутри строки закрывала вызов раньше времени: всё,
 * что стояло за ней, — в том числе ключ, реально уезжающий на сервер, — выпадало из
 * области и пин молчал (дефект самого пина, сданный задачей 6 задаче 7).
 */
export function* managerCalls(src: string): Generator<{ region: string; line: number }> {
  for (const match of src.matchAll(/\bmanagers\.[A-Za-z.\s\n]*?\(/g)) {
    ...
    for (; i < src.length; i++) {
      const ch = src[i]
      if (ch === "'" || ch === '"' || ch === '`') {
        // Пропускаем литерал целиком, вместе с экранированными кавычками внутри.
...
describe('данные — не ключи перевода', () => {
  ...
  // ДЕФЕКТ САМОГО ПИНА, сданный задачей 6 задаче 7: границы вызова считались по
  // балансу скобок, НЕ РАЗЛИЧАЯ скобок внутри строковых литералов. Любая `)` в строке
  // закрывала область раньше времени, и всё, что стояло за ней — включая ключ,
  // реально уезжающий на сервер, — переставало проверяться. Скобки в подписях у нас
  // обычное дело («Delete (all)», «%d file(s)»), так что случай не выдуманный.
  //
  // Проверяется на синтетическом исходнике: на настоящих файлах «правильно» и
  // «сломано» дают один и тот же зелёный, пока такой строки в них не окажется.
  it('скобка внутри строкового литерала не закрывает вызов менеджера', () => {
    // Скобка НЕПАРНАЯ — иначе «правильно» и «сломано» дают одну и ту же область:
    // лишняя пара внутри строки съедает сама себя, и баланс сходится случайно.
    const src = "await managers.groups.createGroup({ about: 'смайлик :)', title: 'NewGroup' })"
    const [call] = [...managerCalls(src)]

    // Область обязана дотянуться до конца вызова, а не оборваться на смайлике.
    expect(call.region.endsWith('})')).toBe(true)
    expect(call.region).toContain("title: 'NewGroup'")
  })
