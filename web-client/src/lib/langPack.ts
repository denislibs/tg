 ...
 * `checkLangPackForUpdates` (tweb :102-128, :170-251, :768-774).
 ...
 * самой функции.
 *
 * ГЛАВНОЕ ПРАВИЛО СЛИЯНИЯ, 1:1 с оригиналом (tweb :238-244): локальный
 ...
 *    :253-261, :696-762) — переехали к владельцу пакета, в менеджер воркера:
 *    они пишут КЭШ и считают ВЕРСИЮ, а ни того, ни другого на вкладке нет.
 *  • `getStrings` (tweb :207-209, доспрос отдельных ключей) — вызывающего нет
 ...
 *    `state_cleared` (tweb :764-766, :777-779): кадров langpack наш сервер не
 ...
 *  • `polyfillPromise` / `./pluralPolyfill` (tweb :263-271): `Intl.PluralRules`
 ...
 *  • `UNSUPPORTED_LANG_PACK_KEY` (tweb :75): ключей `Message.Unsupported.Mobile`
 *    /`.Desktop` в нашем `lang.ts` нет (есть один `Message.Unsupported`), а
 *    вызывающего у константы нет ни одного.
 *  • Сигнал `langCodeNormalized`/`setLangCodeNormalized` (tweb :96, :105):
...
namespace I18n {
  ...
  /** tweb :83 — второго применения на тот же старт не заводим. */
  let cacheLangPackPromise: Promise<LangPackDifference> | undefined

  /**
   * tweb :170-192 — ПУТЬ ПЕРВОГО ЗАПУСКА: строки, вшитые в бандл.
   *
   ...
   * (:118-119); у нас локальные строки в кэш не едут никогда — они всегда лишь
   * НИЖНИЙ СЛОЙ слияния, — поэтому отсюда возвращается ровно то, чем этот слой
   * и является: список строк.
   *
   * Импорт динамический, как у оригинала: словарь на 1302 ключа не должен
  ...
  /** tweb :108-113 — пакет из КЭША, без сети. Параметра `dontLoadLocal`
  ...
  /** tweb :194-205 — серверный пакет языка. Сеть, кэш и запись держит владелец;
  ...
  /**
   * tweb :236-249 — СЛИЯНИЕ И ПРИМЕНЕНИЕ: локальный английский вниз, серверные
   * строки поверх.
   *
  ...
  /**
   * tweb :115-128 — холодный старт: применить то, что уже лежит НА ДИСКЕ.
   *
  ...
  /** tweb :211-231 — плоский объектный словарь (`lang.ts`) в конструкторы схемы.
  ...
  /** tweb :273-338 — применение языка: правила числа, строки, перерисовка живых узлов. */
  export function applyLangPack(langPack: LangPackDifference) {
    ...
    // В оригинале правила числа создаются ДВАЖДЫ (:279-291): сначала по
  ...
  // tweb :349-356. Второй элемент карты у оригинала (звезда) закомментирован — его нет и здесь.
  ...
  /**
   * tweb :358-460 — разбор микроразметки словаря в СПИСОК КУСКОВ.
   *
  ...
  /** tweb :462-492 — строка по ключу: форма числа выбирается ЗДЕСЬ, по `Intl.PluralRules`. */
...
/**
 * tweb :768-774 — фоновая проверка обновлений языка.
 *
 ...
 *
 * Запрошенный язык сверяется ДВАЖДЫ — 1:1 с оригиналом (tweb :700-707), и
 ...
 * :274-277) — он на несовпадении не делает ничего. Но без этой строки наружу
 * уезжал бы пакет, который НЕ ПРИМЕНЁН, и вызывающий не отличил бы его от
 * применённого; заодно не строится впустую нижний слой из 1302 строк.
 */
...
// tweb :684-694 — перечисление через локализованные разделители: «а, б и в».
//
