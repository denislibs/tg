 ...
 * (`applyToCore`): выбор языка живёт там (`localStorage(tg-lang)`) и ТОЛКАЕТСЯ
 * сюда, чтобы второго источника «текущего языка» не появилось. `t()` берёт ту же
 * самую карту строк — оба ответа на один ключ обязаны совпадать, и это под пином
 * (`i18n/index.test.ts`).
 *
 * ЗАГРУЗКА С СЕРВЕРА (`getCacheLangPackAndApply`/`getLangPackAndApply`/
 * `checkLangPackForUpdates`) — по-прежнему БЕЗ ПРОИЗВОДСТВЕННОГО ВЫЗЫВАЮЩЕГО:
 * её включает ЗАДАЧА 9 («useT внутри ходит в I18n.strings»), она же снимает
 * `t()` и мост `applyToCore` вместе с ним.
 *
 * Почему старт загрузки не поставлен здесь и сейчас — НЕ ради экономии запроса
 ...
 * кэше, а интерфейс берёт его из `localStorage(tg-lang)` (`useI18nStore`), и в
 ...
 * задача 8, включает старт задача 9. Мост задачи 7 (`i18n/index.tsx::applyToCore`)
 * этой опасности не несёт: он ТОЛКАЕТ выбор пользователя сюда, а не выводит язык
 * заново.
 * ЦЕНА ЭТОГО РЕШЕНИЯ, названная честно: у подсистемы ЗАГРУЗКИ (кэш, сеть, версия)
 * до сих пор нет ни одного производственного вызывающего, поэтому живой проверки
 * по DoD п.10 для неё не существует — только тесты. Именно это укрытие и спрятало
