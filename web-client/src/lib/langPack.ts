 ...
 * файла, а между запусками — в `localStorage('tg-lang')`, который читает и
 * пишет только `setLangCode` ниже. До задачи 8 ответов было два: ядро выводило
 ...
 * язык.
 *
 * Языка БРАУЗЕРА (`navigator.language`) здесь нет намеренно: у оригинала
 * умолчание — `App.langPackCode` (`en`), интерфейс поднимается на английском,
 * пока язык не выбран. Прежнее угадывание по `navigator` было нашей выдумкой и
 * уехало вместе с владельцем.
 *
...
namespace I18n {
  ...
  let lastRequestedLangCode: string = readStoredLangCode() ?? DEFAULT_LANG_CODE
  ...
  /**
   * tweb :104-108 — «язык теперь такой». ЕДИНСТВЕННЫЙ писатель этого факта.
   *
   * У оригинала функция приватная: писать её некому, кроме загрузчиков того же
   * файла, потому что там ХРАНИЛИЩЕМ выбора служит сам кэш пакета
   * (`saveLangPack` пишет пакет, `getCacheLangPackAndApply` выводит язык из его
   * `lang_code`). У нас кэш пакета лежит за границей контекстов, у владельца в
   * воркере, и читается асинхронно — а «текущий язык» нужен СИНХРОННО, на
   * первом же рендере. Поэтому выбор хранится отдельно (`tg-lang`), и записывать
   * его обязана та же строка, что меняет язык в памяти: иначе хранимое и
   * применённое — снова два факта.
   *
   * Экспорт нужен вызывающим за пределами файла: мост React-стора
   * (`i18n/index.tsx::loadLang` — язык там выбирают чанки словарей, а не сервер)
   * и тест словарей (`i18n/dict.test.ts`), применяющий язык из ФАЙЛОВ.
   */
