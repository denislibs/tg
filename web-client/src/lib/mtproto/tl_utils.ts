...
/*
 * Порт `tweb/src/lib/mtproto/tl_utils.ts` — сериализация и разбор TL на
 * клиенте. Вторая сторона того же кодека, что напечатан у нас на Go
 * (`backend/internal/pkg/tl`), и таблицу конструкторов обе стороны читают из
 * ОДНОЙ схемы.
 *
 * Расхождений с оригиналом два, и оба названы здесь.
 *
 * 1. СТРОГОСТЬ ТИПОВ. У оригинала `strictNullChecks` и
 *    `strictPropertyInitialization` выключены, у нас включены — поэтому в
 *    коде появились `!` и `declare`-инициализация полей. Семантику это не
 *    меняет: там, где стоит `!`, оригинал полагается ровно на то же
 *    (конструктор в таблице есть, маска объявлена раньше своих полей).
 *
 * 2. GZIP. Ветка `gzip_packed` НЕ портирована: это конструктор пространства
 *    MTProto, то есть реквизит транспорта, который программа не копирует
 *    (`docs/readiness/tl-program.md`, «Чего НЕ копируем»). На нашем проводе
 *    такой кадр не появляется — и если появится, лучше внятная ошибка, чем
 *    молчаливо неразобранные байты. Вместе с веткой не понадобилась и
 *    зависимость-распаковщик.
 */
...
class TLSerialization {
  ...
  public storeObject(obj: any, type: string, field?: string) {
    switch(type) {
      ...
      case '#':
        obj ||= 0;
        return this.storeInt(obj, field); // проваливание в 'int' — под strict оно ошибка
      case 'int':
...
class TLDeserialization<FetchLongAs extends Long> {
  ...
  public fetchObject(type: string, field?: string): any {
    ...
    const schema = this.mtproto ? Schema.MTProto : Schema.API;
    let constructorData: MTProtoConstructor = null as unknown as MTProtoConstructor;
    let fallback = false;
    ...
    if(type.charAt(0) === '%') {
      ...
      if(constructorCmp === gzipPacked) { // Gzip packed
        // Расхождение 2 (см. шапку): распаковки нет. Конструктор из
        // пространства MTProto, на нашем проводе не бывает.
        throw new Error('gzip_packed: MTProto-транспорт не портирован');
      }
      ...
      if(!constructorData && this.mtproto) {
        ...
        for(let i = 0, len = schemaFallback.constructors.length; i < len; ++i) {
          if(+schemaFallback.constructors[i].id === constructorCmp) {
            ...
            delete (this as unknown as {mtproto?: boolean}).mtproto;
            fallback = true;
            break;
          }
        }
      ...
      if(!constructorData) {
        ...
        let int1: number | undefined, int2: number | undefined;
        try {
