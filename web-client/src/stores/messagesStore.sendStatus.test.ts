...
describe('messagesStore send status lifecycle', () => {
  ...
  it('optimistic message renders as sending (clock), not sent', () => {
    useMessagesStore.getState().appendOptimistic(String(CHAT), 'hi', ME, 'c1')
    const m = useMessagesStore.getState().byKey[String(CHAT)].msgs[0]
  ...
  it('retryOptimistic clears failed → back to sending', () => {
    useMessagesStore.getState().appendOptimistic(String(CHAT), 'hi', ME, 'c1')
    useMessagesStore.getState().failOptimisticByClient('c1')
    useMessagesStore.getState().retryOptimistic(String(CHAT), 'c1')
    const m = useMessagesStore.getState().byKey[String(CHAT)].msgs[0]
  ...
  it('removeOptimistic drops the failed bubble', () => {
    useMessagesStore.getState().appendOptimistic(String(CHAT), 'hi', ME, 'c1')
    useMessagesStore.getState().failOptimisticByClient('c1')
    useMessagesStore.getState().removeOptimistic(String(CHAT), 'c1')
    expect(useMessagesStore.getState().byKey[String(CHAT)].msgs).toHaveLength(0)
  })
})
