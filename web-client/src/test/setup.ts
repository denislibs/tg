...
// `lottie-web` здесь всё равно замокан (см. выше) — на нём ещё держится
// `mediaEditor/stickerAssets.ts` (Этап 3, произвольные пользовательские
// стикеры, не встроенные ассеты) — содержимое JSON тестам не важно,
// подставляем пустышку только для `/assets/tgs/`, всё остальное идёт
// настоящим fetch.
//
