// Порт tweb `src/helpers/dom/wheelClassifier.ts` — 1:1, только TS-синтаксис
// подогнан под наш strict-конфиг. Отличает «живой» ввод колесом/тачпадом от
// инерционного докрутки (её нельзя трактовать как жест разворачивания сториз).
...
export default class WheelClassifier {
  ...
  push(e: WheelEvent): WheelKind {
    ...
    this.events.push({ t: now, d: Math.abs(e.deltaY) })
