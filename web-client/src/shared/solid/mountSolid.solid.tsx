 ...
 *
 * ── Правило для пропов-хендлов ─────────────────────────────────────────────
 * `createStore` заворачивает своим прокси всё, что похоже на «обычный объект»
 * (прототип ровно `Object.prototype` либо массив), и по дороге спрашивает у
 * значения служебные символы `$RAW`/`$PROXY`/`$NODE`, а свои узлы кладёт на
 * него через `Object.defineProperty`. Обычные данные это переживают, а вот
 * ХЕНДЛ (стабильная ссылка на чужую машинерию — RPC-прокси менеджеров и
 * подобное) под такой обход не рассчитан: на нём экран входа гас дважды
 * (PR #237). Инстансы классов и DOM-узлы — а через мост едут и они
 * (`self: this` у `scaffoldSolidJSTab`, `avatarsInfo` у `peerProfile`) — под
 * эвристику не попадают и едут ПО ССЫЛКЕ; значит и рукодельный хендл обязан
 * не выглядеть plain-объектом. Как это сделано у прокси менеджеров и почему
 * это правило хендла, а не моста, — `rpc/managersProxy.ts`, `HANDLE_PROTO`.
 *
