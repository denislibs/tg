// Порт tweb `src/components/swipeHandler.ts` — 1:1 по логике (vanilla TS без
// фреймворка): пороги, wheel-зум (`clamp(deltaY,-25,25)*0.01`, трекпад-пинч
// через `ctrlKey` и «двойной тап» через `-0`-дельты), пинч двумя касаниями
// (центр = середина), mouse-drag с курсором, `verifyTouchTarget`, ленивое
// связывание end-листенера на `element.ownerDocument` (поддержка Document PiP).
//
// Адаптации (только импорты и мёртвый код, поведение не менялось):
//   • `cancelEvent`/`safeAssign`/`clamp`/`windowSize`/`Middleware`/`logger` —
//     уже есть в проекте (порты/шимы tweb по тем же путям `@helpers`/`@lib`)
...
//   • строгий tsconfig (в tweb `strict` выключен): сигнатуры колбэков
//     объявлены в `SwipeHandlerOptions`, а поля класса ссылаются на них (в
//     tweb наоборот — options индексирует ПРИВАТНЫЕ поля класса, наш tsc даёт
//     TS2341); поля, заполняемые `safeAssign`/`resetValues`, помечены `!`
//     (всегда заданы после конструктора) или `?:` (в рантайме честно бывают
//     `undefined` — tweb сам пишет в них `undefined` в `resetValues`)
//     `WheelEvent` в wheel-обработчиках передаётся дальше как `EE` через
//     `as any as EE` (его `target: EventTarget | null` не совместим с `E`
//     под `strictNullChecks`, TS2345 — рантайм тот же, что в tweb).
//
// * zoom part from WebZ
...
// Свайп трекает move/end на СОБСТВЕННОМ документе элемента
...
export type SwipeHandlerOptions = {
  element: HTMLElement,
  onSwipe: (xDiff: number, yDiff: number, e: EE, cancelDrag?: (x: boolean, y: boolean) => void) => boolean | void,
  verifyTouchTarget?: (evt: EE) => boolean | Promise<boolean>,
  onFirstSwipe?: (e: EE) => void,
  onReset?: (e?: Event) => void,
  onStart?: () => void,
  onZoom?: (details: ZoomDetails) => void,
  onDoubleClick?: (details: { centerX: number, centerY: number }) => void,
  cursor?: 'grabbing' | 'move' | 'row-resize' | 'col-resize' | 'nesw-resize' | 'nwse-resize' | 'ne-resize' | 'se-resize' | 'sw-resize' | 'nw-resize' | 'n-resize' | 'e-resize' | 's-resize' | 'w-resize' | '',
  cancelEvent?: boolean,
...
export default class SwipeHandler {
  private element!: SwipeHandlerOptions['element']
  private onSwipe!: SwipeHandlerOptions['onSwipe']
  private verifyTouchTarget?: SwipeHandlerOptions['verifyTouchTarget']
  private onFirstSwipe?: SwipeHandlerOptions['onFirstSwipe']
  private onReset?: SwipeHandlerOptions['onReset']
  private onStart?: SwipeHandlerOptions['onStart']
  private onZoom?: SwipeHandlerOptions['onZoom']
  private onDoubleClick?: SwipeHandlerOptions['onDoubleClick']
  private cursor!: NonNullable<SwipeHandlerOptions['cursor']>
  ...
  private releaseWheelDrag!: DebounceReturnType<(e: Event) => void>
  private releaseWheelZoom!: DebounceReturnType<(e: Event) => void>
  ...
  public setListeners() {
    if(!IS_TOUCH_SUPPORTED) {
      ...
      // Листенер конца жеста (`mouseup`/`touchend`) вешается лениво в
      // `handleStart` (см. комментарий там), не здесь: целевой документ
      // резолвится, пока элемент живой.
      ...
      // `touchend` вешается лениво в `handleStart` (см. комментарий там), не здесь.
    }
  }
  ...
  protected handleStart = async(_e: EE) => {
    ...
    // Листенер конца жеста вешается на ТЕКУЩИЙ документ элемента именно
    // сейчас — пока элемент живёт в DOM, — а не в конструкторе: элемент,
    // клонированный из <template>, при создании принадлежит инертному
    // документу template-содержимого (about:blank), и повешенный туда
    // `mouseup`/`touchend` реального отпускания (оно придёт на главный — или
    // Document-PiP — документ) не увидит: жест встал бы по нажатию и никогда
    // не завершился. Резолв `ownerDocument` здесь же сохраняет поддержку PiP.
    // Делается ДО (возможно асинхронного) verify, чтобы отпускание во время
    // проверки тоже чисто сбрасывало жест.
    if(this.endListener) {
      this.listenerSetter.remove(this.endListener)
    }
    // @ts-ignore
  ...
  protected handleWheel = (e: WheelEvent) => {
    if(!this.hadMove && this.verifyTouchTarget) {
      const result = this.verifyTouchTarget(e as any as EE)
  ...
  protected handleWheelDrag = (e: WheelEvent) => {
    ...
    void this.releaseWheelDrag(e)
    this.dispatchOnSwipe(x, y, e as any as EE, (dx, dy) => {
  ...
  protected onWheelCapture = (e: WheelEvent) => {
    ...
    this.setHadMove(e as any as EE)
