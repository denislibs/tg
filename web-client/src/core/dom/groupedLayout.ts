// Порт tweb components/groupedLayout.ts (сам — порт tdesktop
// ui/grouped_layout.cpp): раскладка альбома 2..10 медиа в грид по их
// пропорциям. Логика 1:1, помощники accumulate/clamp инлайнены.

type Size = { w: number; h: number }

export interface GroupMediaLayout {
...
const accumulate = (arr: number[], initial: number) => arr.reduce((a, b) => a + b, initial)
...
export class Layouter {
  ...
  constructor(
    ...
    this.averageRatio = accumulate(this.ratios, 1) / this.count
  ...
  private layoutThreeLeftAndOther(): GroupMediaLayout[] {
    ...
    const rightWidth = Math.max(
      this.minWidth,
      Math.round(Math.min((this.maxWidth - this.spacing) / 2, Math.min(thirdHeight * this.ratios[2], secondHeight * this.ratios[1]))),
    )
  ...
  public layout(): GroupMediaLayout[] {
    ...
    for (let first = 1; first !== this.count - 1; ++first) {
      for (let second = 1; second !== this.count - first; ++second) {
        ...
        if (first > 3 || second > (this.averageRatio < 0.85 ? 4 : 3) || third > 3) {
          continue
        }
