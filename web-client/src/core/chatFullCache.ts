...
//
// Зеркало, а не второй источник: единственные писатели — загрузчики карточек
// (`useChatInfoCard` → `groups.card`, `Chat.tsx` → `privacy.profile`) и
// проектор кадра `chat_theme_update`. Механика подписки — та же, что у
// `core/peerCache.ts`: счётчик версии + `useSyncExternalStore`, потому что
// снимок это КОЛЛЕКЦИЯ, а не значение по ключу.
//
...
export function saveChatFull(peerId: PeerId, full: PeerFull): void {
