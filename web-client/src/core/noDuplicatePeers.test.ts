...
describe('зеркало пиров: воркер владеет карточками, витрина только зеркалит', () => {
  ...
  //
  // Операция теперь ОДНА (`upsert`). Прежний `patch` (точечные поля известной
  // карточки) исчез вместе с кадром, который его порождал: `user_update` несёт
  // конструктор `user` ЦЕЛИКОМ — абсолютный снимок, — а не выборку изменившихся
  // полей рядом с флажком `avatar_changed`.
