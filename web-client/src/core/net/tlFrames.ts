...
/**
 * Разбирает тело ВИТРИНЫ REST, приехавшее проводом TL.
 *
 * Живёт здесь, а не рядом с `restClient`, ровно из-за переходника
 * `bytesToBase64`: два его экземпляра означали бы два ответа на вопрос «в какой
 * форме едут байты», и они бы разъехались.
 *
 * Тип не передаётся: `fetchObject('Object')` опознаёт и конструктор, и вектор
 * по id из потока — так же поступает оригинал, у которого на этом месте стоит
 * фолбэк `'Object'` (`networker.ts:1503`). Ожидаемый тип нужен лишь голым
 * примитивам, а их наши витрины на провод не выпускают.
 */
export function decodeTLValue(raw: Uint8Array): unknown {
  return bytesToBase64(new TLDeserialization(raw).fetchObject('Object', 'rest'))
}
