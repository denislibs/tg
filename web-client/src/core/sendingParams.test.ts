...
const ALLOWED: Record<string, number> = {
  ...
  // Переотправка упавшего бабла: параметры восстанавливаются из САМОГО
  // сообщения (`replyToMsgId: raw.replyToId`, `threadId: raw.threadRootId`), а
  // не из текущего инпута — порт tweb `repayCallback`
  // (appMessagesManager.ts:1484-1489: повтор идёт тем же `options`, что и первая
  // попытка). Взять здесь пакет композера было бы ОШИБКОЙ: он описывает то, что
  // пользователь набирает СЕЙЧАС, а повторяем мы отправку минутной давности.
  'core/hooks/useMessageActions.tsx': 1,
