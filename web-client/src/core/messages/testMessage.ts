...
export function makeRawMessage(f: MessageFixture): RawMessageReal {
  ...
  // Приведение — из-за НЕДОВЕДЁННОГО объединения MessageMedia: у модели гео,
  // опрос и превью ссылки уже разобраны (`GeoData`/`Poll`/`WebPageData`), а на
  // проводе едут своими формами. Фикстура ни одного из них не ставит, поэтому
  // расхождение здесь чисто типовое.
