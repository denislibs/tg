...
// (у нас всюду число), а `dialog.draft` тянет за собой полный `DraftMessage`
// оригинала (нашему хватает `message`/`entities`/`reply_to`/`date` — остальные
// параметры предмета не имеют, см. `core/models.ts`).
