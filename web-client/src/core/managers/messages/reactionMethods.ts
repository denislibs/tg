...
export function newReactionMethods({ rest, patchMsg, getMeId, getMePremium, opWindowsFor, emitOps, readMsg, peers }: MessagesCtx) {
  ...
  //
  // Дельт бывает НЕСКОЛЬКО на один клик — вытеснение своих реакций сверх лимита
  // идёт теми же дельтами, — и объявляются они ОДНОЙ операцией: у оригинала
  // весь набор моих реакций тоже меняется разом (`messages.sendReaction` с
  // массивом, appReactionsManager.ts:948-951). Объявить их по одной значило бы
  // пересобрать ряд реакций столько раз, сколько было дельт.
  const applyLocalDeltas = (
    peerId: number,
    msgId: number,
    me: PeerId,
    deltas: { emoji: string; action: 'add' | 'remove' }[],
  ): void => {
    const id = generateMessageId(msgId)
    let applied = false
    for (const { emoji, action } of deltas) {
  ...
  const applyLocalDelta = (peerId: number, msgId: number, emoji: string, action: 'add' | 'remove', me: PeerId): void =>
    applyLocalDeltas(peerId, msgId, me, [{ emoji, action }])
  ...
  //
  // ЭХО СВОЕГО КЛИКА кадром НЕ ПЕРЕСОБИРАЕТ ряд реакций. Кадр реакции сервер
  // шлёт всем членам чата, включая автора клика (backend
  // internal/usecase/chat/reaction.go), поэтому через 10-130 мс после
  // оптимистичной дельты приходит абсолютный агрегат, описывающий то же самое
  // состояние. Пока слияние всегда возвращало новый объект, окно получало
  // вторую операцию `patch` — а лента на неё пересобирает ряд целиком и
  // выбрасывает из документа узел чипа, вокруг которого В ЭТОТ МОМЕНТ летит
  // эффект реакции (`components/wrappers/stickerAnimation.ts` снимает полёт по
  // «узла нет в документе»). Отсюда и была жалоба «анимация то играет, то нет»:
  // успеет ли `around.tgs` догрузиться до второго пересбора, решал кэш.
  //
  // Сравнивается ВИДИМОЕ (`sameReactions`), а не ссылка: у кадра и локальной
  // дельты объекты разные по построению.
  const applyAbsoluteReactionToCache = (evt: ReactionEvt): MessageOp[] => {
    ...
    patchMsg(peerId, (m) => m.id === id, (m: MyMessage) => {
      const merged = mergeReactions(m.reactions, evt.reactions)
      if (sameReactions(m.reactions, merged)) return null
      changed = true
      return { ...m, reactions: merged }
    })
    return changed ? reactionOps(peerId, id) : []
  }
  ...
  return {
    ...
    async react(peerId: number, msgId: number, emoji: string): Promise<void> {
      ...
      const serverId = getServerMessageId(msgId)
      // ЛИМИТ СВОИХ РЕАКЦИЙ. Своих реакций на сообщении бывает не больше
      // `reactions_user_max_*`; при переполнении снимается самая старая, молча
      // (порт tweb appReactionsManager.ts:733-751). Снятие идёт ТОЛЬКО локально:
      // на проводе у оригинала один запрос со ВСЕМ набором (:948-951), а у нас
      ...
      if (me != null) {
        applyLocalDeltas(peerId, serverId, me, [
          ...evicted.map((e) => ({ emoji: e, action: 'remove' as const })),
          { emoji, action: 'add' as const },
        ])
      }
      try {
        await rest.post(`/chats/${peerId}/messages/${serverId}/reactions`, { emoji })
      } catch (e) {
        // Откат — обратной пачкой, тоже одной операцией. Вытесненные
        // возвращаются последними в моём порядке, а не на своё прежнее место:
        // прежних номеров уже нет, а следующий кадр всё равно приедет с
        // серверным составом.
        if (me != null) {
          applyLocalDeltas(peerId, serverId, me, [
            { emoji, action: 'remove' as const },
            ...evicted.map((e) => ({ emoji: e, action: 'add' as const })),
          ])
        }
        throw e
      }
