 ...
 * У tweb пакет лежит на ВКЛАДКЕ: кэш — `commonStateStorage` (:110, :257), а
 * версию и разницу считают модульные функции рядом с `I18n` (:696-774). Вкладок
 ...
 * (локальный английский + серверные строки, :238-244), потому что слияние там
 ...
 * выигрыш: пакет в кэше не раздут 1302 английскими строками, которые и так
 * лежат в бандле.
 *
 ...
 *    `langPack.ts:194-205, 238-244`). У наших маршрутов места для него нет
...
export function newLangPackManager({ rest, kv }: LangPackDeps) {
  /**
   * Полёты по ключу. У tweb ту же роль играет `cacheLangPackPromise` (:83,
  ...
  /** Порт `saveLangPack` (:253-261). Отказ записи глотаем по той же причине,
  ...
  /** Порт `loadLangPack` (:194-205) + `langpack.getLangPack`: весь пакет языка.
  ...
  return {
    ...
    /**
     * Порт `getCacheLangPack(true)` (:108-113): пакет из кэша, БЕЗ СЕТИ.
     ...
     * Порт `getLangPackAndApply` в части загрузки (:233-235) — применение
    ...
    /**
     * Порт `checkLangPackForUpdates` (:768-774): спросить разницу от версии
     ...
     * `I18n.getLastRequestedLangCode()` ДВАЖДЫ (:700-707) — до чтения кэша и
    ...
    checkForUpdates(langCode: string): Promise<LangPackDifference | null> {
      return once(`diff:${langCode}`, async () => {
        const stored = await cached()
        // Первая сверка (tweb :700): кэш не про запрошенный язык — разницу не
        ...
        // Вторая сверка (tweb :705-706): пока разница летела, кэш мог смениться
