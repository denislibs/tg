...
describe('mediaUrl — зеркало токена владельца', () => {
  ...
  // Оставшийся потребитель токена среди компонентов (Task 7 увёл картинки на
  // useMediaUrl) — RealMediaBubble (инлайн-автоплей <video> и href документа)
  // устроен так же: подписался, спросил hasMediaToken() и при «нет» вернул
  // подложку — mediaContentUrl он при этом НЕ зовёт. Значит запрос из
  // mediaContentUrl его не спасает, и единственный, кто в этом состоянии идёт
  // к владельцу, — сама подписка. Пин НЕ ослабляем: стрим при DNP-OFF всё ещё
  // на токене.
