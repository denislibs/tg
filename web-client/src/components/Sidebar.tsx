...
function ArchiveList({ chats, selectedId, onSelect }: {
  ...
  // Обёртки строк, размер набора, признак «хоть раз загружались» и запрос
  // страницы — всё из общего источника списка диалогов (там же живут и кэш
  // обёрток, без которого `useShouldAnimate` сравнивал бы по ссылке всегда
  // разные элементы, и правило принадлежности строки выборке).
