...
export default function useEmoticonsStickySpy({
  ...
  // Остров: StickyIntersector досыпает сентинелы внутрь категорий и не убирает
  // их за собой (в tweb контейнер одноразовый) — уборку берёт на себя `strays`,
  // иначе пересоздание наблюдателя дублирует их в каждой категории.
  useImperativeIsland((sc) => {
    ...
    for (const el of catEls.current.values()) si.observeStickyHeaderChanges(el)
  ...
  }, [scrollRef, catEls, count], { host: scrollRef, strays: '.sticky_sentinel' })
