...
export default function SignInCard(_props: { spec: Spec }): JSX.Element {
  ...
  const onPhoneInput = (raw: string) => {
    ...
    //
    // Расширение против буквального `telInputField.ts:70` (raw==='' туда НЕ
    // попадает: `''.replace(/\++/, '+')` не находит `+` и возвращает исходную
    // ''`, formatPhoneNumber('') отдаёт formatted='' — код страны выкусывается
    // подчистую). Полностью пустой ввод обрабатываем СИМВОЛОМ ТОЙ ЖЕ ветки, что
    // и одинокий «+»: код страны не должен становиться выкусываемым удалением
    // ниже минимума — набор нового номера начинается СО ЗНАКА «+», а не с
    // абсолютно пустого поля без единственной подсказки, что это телефон.
    if (raw === '' || raw.replace(/\++/, '+') === '+') {
      setPhone('+')
