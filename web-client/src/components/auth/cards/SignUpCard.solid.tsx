...
//
// ── Интерактивный кроппер — ВНЕ периметра (сознательное сужение) ───────────
...
// пускает React), а порт кроппера на Solid — самостоятельная задача, которой
// нет ни в брифе, ни в списке файлов задачи 5/6. Снести сам React-компонент
// при этом НЕЛЬЗЯ: он живой и нужен другим React-экранам (`EditProfile`/
// `NewGroupFlow`/`EditContactView`/`GroupEditFlow`, ещё не портированным на
// Solid), а обратного моста «React внутри Solid» в проекте нет (мост только
// в одну сторону — `<SolidIsland>`, см. `web-client/CLAUDE.md`); заводить его
// ради одной карточки было бы архитектурной отсебятиной сверх периметра. Долг
// назван записью — `web-client/backlogs/frontend/avatar-cropper-solid-port.md`
// (число «ЗАДАЧА #NNN» здесь раньше указывало на чужой долг — тот же номер
// уже занят `stores/audioStore.ts`; ссылка на файл не коллизирует). До порта
// выбранный файл здесь не кадрируется и не уменьшается интерактивно —
// центр/масштаб задаёт исходное фото целиком, а не пользователь; `<canvas
// id="canvas-avatar">` рисует ЛУЧШЕЕ ИЗ ВОЗМОЖНОГО live-превью (best-effort
// через `Image.onload`, декодирование не гарантировано во всех окружениях и
// не участвует в данных отправки — если предпросмотр не нарисовался, аватар
// всё равно загрузится).
//
...
// `media.upload`. НЕ «на выходе всегда jpeg»: если браузер не смог декодировать
// файл (`createImageBitmap` бросил — типичный случай для HEIC везде, кроме
// Safari), `scaleImageForSend` отдаёт ОРИГИНАЛ как есть, с `width`/`height =
// 0` (её собственный докблок, ветка `catch`) — тот же исходный регресс на
// недекодируемых форматах остаётся, просто сузился до окружений, где браузер
// в принципе не умеет такой файл открыть. Прежний регресс (сравнение с React
// `AvatarCropper`, который СЧИТАЛ width/height САМ) закрыт для ДЕКОДИРУЕМЫХ
// форматов; интерактивный кроп/зум/пан остаётся отдельным долгом — см. ссылку
// на backlog выше.
//
