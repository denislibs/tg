// Порт tweb `src/components/preloader.ts` (ProgressivePreloader) — 1:1 по
// логике: SVG-кольцо прогресса (`totalLength = 149.82473754882812`, стримовое
// `118.61124420166016`), `TRANSITION_TIME = 200`, состояния
// вращение/детерминированный `setProgress` (strokeDasharray)/`setManual`,
// отмена по клику (`promise.cancel`), ретрай (`tryAgainOnFail` → `loadFunc`),
// `attach`/`detach` через SetTransition (`is-visible`, detach после 100% —
// через `TRANSITION_TIME · 0.75` = 150 мс), `tempId`-гард от протухших
// промисов. Стили — `styles/tweb/_preloader.scss` (порт целиком уже в дереве).
//
// Адаптации (поведение не менялось):
//   • RTMP-вариант (`options.rtmp`, `preloader-container-rtmp`, разметка
//     `loading-wrap`) не портирован — у нас нет RTMP-трансляций; вернуть из
//     tweb preloader.ts:56-58,85-92 при появлении фичи
//   • `SetTransition` — наш именованный порт `setTransition`
//     (`@core/dom/setTransition`, та же машина классов), `useRafs` довезён
//     туда вместе с этим файлом
//   • приватные `cancelSvg`/`downloadSvg` — только присваивались и в tweb
//     (нигде не читаются), у нас это ошибка сборки (noUnusedLocals) — убраны
//   • строгий tsconfig (в tweb `strict` выключен): `preloader`/`circle` —
//     `!` (заданы после `construct`), `promise` хранится как
//     `CancellablePromise<unknown>`, а `attachPromise` — дженерик (иначе
//     контравариантность полей-колбэков не пропустит `CancellablePromise<T>`
//     конкретного T); `construct` — стрелка-поле с типом `| null` (tweb
//     присваивает null методу); `onEnd(err: Error | null)`;
//     `this.promise = promise = null` разбит — обнуление локали в tweb лишь
//     GC-хинт, локаль умирает вместе с замыканием
//   • innerHTML здесь — статические строки tweb (SVG-иконки крест/стрелка,
//     кольцо), пользовательских данных нет — норма безопасности про
//     user content не нарушается
...
export default class ProgressivePreloader {
  ...
  public promise: CancellablePromise<unknown> | null = null
  ...
  public construct: (() => void) | null = () => {
    ...
    // Константы tweb (getTotalLength кольца); no-loss-of-precision — ложное
    // срабатывание на 17 значащих цифрах: Number(литерал).toString() возвращает
    // его же, double хранит значение точно
    if(this.streamable) {
  ...
  public attachPromise<T>(promise: CancellablePromise<T>) {
    if(this.isUpload && this.promise) return

    this.promise = promise as CancellablePromise<unknown>
    ...
    const onEnd = (err: Error | null) => {
      ...
      if(!err && this.cancelable) {
        ...
        if(elapsedTime < delay) {
          ...
          setTimeout(() => { // * дождаться завершения перехода дуги к 100%
            if(tempId === this.tempId) {
              this.detach()
            }
          }, delay)
    ...
    promise.addNotifyListener?.((...args: unknown[]) => {
      ...
      const details = args[0] as { done: number, total: number }
      const percents = details.done / details.total * 100
      this.setProgress(percents)
    })
  }

  public attach<T>(elem: Element, reset = false, promise?: CancellablePromise<T>) {
