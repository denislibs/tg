...
// (`appSearchSuper.cleanupHTML()` — tweb `appSearchSuper.ts:2780-2786`).
//
...
// (`docs/tweb/dom/dumps/07-right-sidebar.json:249-251`).
//
// `setButtonLoader` и `PreloaderTsx` (`putPreloader.ts:29-54`) не портированы:
 ...
 *
 * Ветка `if(elem)` оригинала (`:16`) не портирована: у `elem` там тип `Element`,
