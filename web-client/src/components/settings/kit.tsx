...
export function Row({
  ...
  const title = (
    <div className={classNames('row-title', multiline ? 'pre-wrap' : '')}>
      {/* РАСКОЛ КОНТРАКТА, ОСТАВШИЙСЯ У REACT-СТОРОНЫ: при `translate` здесь ключ, без
          него — готовый текст (имя пира, номер телефона), и по типу они не различаются.
          Приведение честнее молчаливого `string`: ключ обязан быть ключом. Задача 7
          сняла раскол в ВАНИЛЬНОМ слое (там роль поля выражена типом); здесь его
          снимает переезд экрана на Solid — до тех пор `t()` жив (задача 9). */}
