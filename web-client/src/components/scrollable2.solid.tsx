 ...
 * `ScrollableX`, класс — порт `tweb/src/components/scrollable.ts`, владеет
 * скроллом ленты сообщений, см. раздел «Скролл» в `web-client/CLAUDE.md`).
 * Этот файл — независимый Solid-рендер того же поведения, ровно как у
 * `buttonTsx.solid.tsx`/`rowTsx.solid.tsx`/`section.solid.tsx`: он не
 * оборачивает `scrollable.ts` и не сводится к нему — throttle/триггеры/палец
 * пересчитаны как Solid-сигналы и мемо, а не поля класса. Арифметика
 * (throttle, `updateThumb`, `checkForTriggers`, направление скролла) СОВПАДАЕТ
 * дословно с `scrollable.ts`, потому что там она уже сверена с оригиналом;
 * два порта одного файла tweb неизбежно держат одну и ту же математику дважды
 * — так же, как у Row/Section держат одну разметку. Классы узла те же, что у
 * `.scrollable`/`.scrollable-thumb` из императивного порта, поэтому
 * `styles/tweb/_scrollable.scss` обслуживает оба.
 *
 * ── Отличия от оригинала (тот же список правок, что уже применён к
 * `components/scrollable.ts` — см. его шапку) ──────────────────────────────
 *  • `useHeavyAnimationCheck` — `onHeavyAnimation` из `@core/dom/heavyAnimation`
 *    (переименован через `as`, тело не менялось): это ЕДИНСТВЕННАЯ шина
 *    тяжёлых анимаций в репо, на неё уже подписан
 *    `components/animationIntersector.ts`; заводить вторую, несвязанную шину
 *    ради этого порта означало бы, что скролл карточек авторизации глушил бы
 *    стикеры в СВОЁМ инстансе, а `animationIntersector` слушал бы другой.
 ...
 *
 * Вычет: у оригинала есть закомментированный (мёртвый) параметр `padding`
 * (`scrollable.ts::padding`, векторный кусок «withPaddingContainer») — в
 * `scrollable2.tsx` его нет вовсе, переносить нечего.
 */
import {
  children,
  createContext,
...
export default function Scrollable(props: {
  ...
  const onThumbMouseUp = (_e: MouseEvent) => {
