// Порт tweb `components/scrollable.ts` — 1:1 по логике; правки только под
// наш tsconfig/oxlint и там, где у нас физически нет зависимости. Английские
// комментарии tweb (включая закомментированные черновики/консольные скрипты)
// сохранены как есть.
//
// Отличия от tweb:
//   • `useHeavyAnimationCheck` импортируется из `@core/dom/heavyAnimation`
//     (`onHeavyAnimation`, переименован через `as` — тело файла не менялось),
//     а не из `@hooks/useHeavyAnimationCheck` — у нас это ЕДИНСТВЕННАЯ шина
//     тяжёлых анимаций (на неё уже подписан `animationIntersector`), тот же
//     путь фикса уже применён в `helpers/fastSmoothScroll.ts` (см. шапку того
//     файла) — заводить вторую, несвязанную шину означало бы, что скролл
//     глушил бы стикеры в СВОЁМ инстансе, а `animationIntersector` слушал бы
//     другой. Сигнатура идентична: `(onStart, onEnd) => () => void`;
//   • `strictPropertyInitialization`/`strictNullChecks` у нас включены (в
//     tweb `strict` выключен, см. TWEB/tsconfig.json) — поля, не
//     инициализируемые в конструкторе, помечены `!` (тот же приём, что в
//     `helpers/scrollSaver.ts`): `scrollPositionProperty`/`scrollSizeProperty`/
//     `clientSizeProperty`/`offsetSizeProperty`/`clientAxis` (проставляются в
//     конструкторе ПОДКЛАССА — `Scrollable`/`ScrollableX` — a не
//     `ScrollableBase`, TS не видит это через границу класса),
//     `startMousePosition`/`startScrollPosition` (пишутся в `onMouseDown` до
//     первого чтения в `onMouseMove`), `thumb`/`thumbContainer` (создаются
...
//     нигде — код, заводящий его, закомментирован; `splitUp` принимает
//     `undefined` через `setVirtualContainer(undefined)`) — под
//     strictNullChecks `!` тут не подходит (он только про инициализацию в
//     конструкторе, не про допустимость `undefined` дальше), поэтому все
//     эти поля объявлены `?:` — не меняет рантайм-семантику (везде и так
//     стоит проверка перед использованием), только честно отражает тип;
//   • неиспользуемые параметры под `noUnusedParameters`: `withPaddingContainer`
//     в конструкторе `Scrollable` переименован в `_withPaddingContainer` (его
...
export class ScrollableBase {
  ...
  protected onMouseUp = (_e: MouseEvent) => {
