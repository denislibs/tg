...
/**
 * Держатели класса `tgico` в JSX (React-сторона). Узел там строит React, а не
 * наша функция, поэтому это не копии `Icon()`, но список ведём: рост = кто-то
 * рисует иконку в обход `<TgIcon>`/`Icon()`. Путь относительно `src/` → число
 * вхождений; правь список руками, а не подгоняй код под тест.
 */
