...
describe('Chat.tsx — монтирование императивной ленты', () => {
  ...
  // Пороги автозагрузки медиа — порт `chat.autoDownload` (tweb chat.ts:137,
  // считается в `setPeer` :1055). Их считает ЭТОТ экран (лента про сторы и вид
  // пира не знает), а раздаёт врапперам лента. Не передать = врапперы качают
  // всё, и настройка «Автозагрузка медиа» перестаёт что-либо значить.
  it('лента получает свод автозагрузки', () => {
    expect(FEED_JSX).toContain('autoDownload={autoDownload}')
  })
