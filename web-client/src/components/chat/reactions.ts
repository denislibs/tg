 ...
 *    — не блокер.
 *    Настоящий блокер — второй источник генерика, набор самих generic-
 *    анимаций: бэк отдаёт только роли ОДНОЙ реакции
 *    (`domain/available_reaction.go`), стикерсетов по короткому имени
 *    (`inputStickerSetEmojiGenericAnimations` оригинала) у него нет вовсе —
 *    это бэкенд-работа за периметром фронтового Этапа 5, не «малая правка».
