// Порт tweb `src/components/mediaViewer/snapshotSize.ts` — дословно
// (зависимостей нет, константы `MAX_DEVICE_PIXEL_RATIO`/`MAX_PIXEL_AREA` не
// менять); правки только под формат `.oxlintrc.json` (без `;`) и строгий
// tsconfig (в tweb `strict` выключен): опциональные `sourceWidth`/
// `sourceHeight` сравниваются и читаются через `!` — голое tweb-сравнение
// `undefined > 0` даёт TS18048/TS2532, рантайм с `!` тот же (false → фолбэк
// на display-размеры).
