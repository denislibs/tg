// Порт tweb `src/tests/mediaViewerSnapshotSize.test.ts` — 1:1 (готовые
// фикстуры поведения из tweb). Адаптации: явный импорт из vitest (в tweb —
// jest-глобали), формат без `;` по `.oxlintrc.json`; файл лежит рядом с
// модулем (наша раскладка тестов — колокейтед), импорт поэтому относительный.
