// Вендор 1:1 из tweb `src/vendor/solid-transition-group/common.ts`, который сам
// является форком https://github.com/solidjs-community/solid-transition-group.
// Портируем только то, что реально использует `AuthCardsHost.solid.tsx` —
// `<Transition>` (не `<TransitionGroup>`, у неё свой набор классов/раскладка
// для СПИСКОВ элементов, предмета для неё в auth-флоу нет; появится
// потребитель — довенднрить `transitionGroup.ts` тем же приёмом).
//
// Логика (классы enter/exit, порядок событий, editableFrame-манёвр Firefox)
// не тронута ни на строку — это общий примитив для ОБОИХ компонентов
// оригинала, менять его в отрыве от `<TransitionGroup>` означало бы держать
// два разных поведения под одним именем.
import { createMemo } from 'solid-js'
import type { TransitionEvents, TransitionProps } from './transition'

export function createClassnames(props: TransitionProps) {
  return createMemo(() => {
