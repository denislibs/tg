// Вендор 1:1 из tweb `src/vendor/solid-transition-group/transition.ts` (сам
// файл — форк https://github.com/solidjs-community/solid-transition-group).
// Единственная правка — путь импорта `common` (относительный, а не через
// `@vendor/solid-transition-group/common`: тот алиас указывал бы сам на себя).
// `@solid-primitives/transition-group`/`@solid-primitives/refs` — те же версии,
// что в tweb `package.json` (`^1.1.2`/`^1.1.4`), настоящие npm-зависимости, а
// не вендор: у tweb они тоже обычные `dependencies`, вендорится только сама
// обёртка `solid-transition-group`.
