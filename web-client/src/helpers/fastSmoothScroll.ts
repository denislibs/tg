// Порт tweb `helpers/fastSmoothScroll.ts` — 1:1 по логике; правки только под
// наш tsconfig и под наш oxlint (`.oxlintrc.json`: без `;`, чинится
// `oxlint --fix`; логика не менялась ни на строку) и под `strictNullChecks`
// (у нас включён, в tweb выключен — см. TWEB/tsconfig.json): все обращения к
// `margin`/`maxDistance` внутри `scrollWithJs` получили non-null assertion
// (`margin!`/`maxDistance!`) — оба опциональны по типу `ScrollOptions`, но
// `fastSmoothScroll()` всегда проставляет дефолт (`options.margin ??= 0`,
// `options.maxDistance ??= LONG_TRANSITION_MAX_DISTANCE`) до вызова
// `scrollWithJs`; тот же приём, что уже применён в этом файле для
// `let path!: number`. `let jumpToScrollPosition: number` расширен до
// `number | undefined` — код намеренно возвращает его в `undefined`
// (`jumpToScrollPosition = undefined` ниже по файлу).
//
// Зависимость `@helpers/animation` (animateSingle/cancelAnimationByKey) и
// `@helpers/dom/isInDOM` в брифе Задачи 2 явно не названы, но чисты — довезены
// той же задачей.
//
// `dispatchHeavyAnimationEvent` импортирован из `@core/dom/heavyAnimation`, а
// не из `@hooks/useHeavyAnimationCheck` (как в tweb) — у нас `hooks/
// useHeavyAnimationCheck.ts` уже портирован раньше именно туда (см. шапку
...
// animationIntersector слушал бы другой — эффекта паузы на скролле не было
// бы вообще. Сигнатура совпадает (`dispatchHeavyAnimationEvent(promise:
// Promise<unknown>, timeout?): Promise<void>`), правка — только путь импорта.
