// Порт tweb `helpers/scrollSaver.ts` — 1:1 по логике; правки только под наш
// tsconfig/oxlint и там, где у нас физически нет зависимости. Английские
// комментарии tweb (JSDoc-подобные, закомментированные альтернативные
// реализации) сохранены как есть.
//
// Отличия от tweb:
//   • конструктор в tweb принимает `Scrollable | ScrollableContextValue` —
//     класс/solid-компонент с кастомным (перерисовываемым) скроллбаром и
//     полем `container`. Полный порт `Scrollable` — отдельная задача этого
//     этапа (Задача 4, см. task-2-report.md/progress.md), не эта. У нас лента
//     скроллится нативным div'ом напрямую (никакого JS-thumb — `_scrollable.scss`
//     это чистый CSS), поэтому вместо импорта обоих типов заведён локальный
//     `ScrollSaverTarget` — ровно то подмножество полей `ScrollableContextValue`
//     (tweb `components/scrollable2.tsx`), которым реально пользуется код
//     ниже. Тонкий адаптер поверх scrollRef собирает `useChatScroll.ts` на
//     месте использования;
//   • `onSizeChange` в tweb перерисовывает кастомный скроллбар-thumb — у
//     адаптера `useChatScroll.ts` это no-op (нативный скролл, рисовать
//     нечего);
...
//     Scrollable не среагировал на программную запись. У нас в `useChatScroll.ts`
//     ВСЕ остальные программные записи `scrollTop` (пин к низу, компенсация
//     paddingTop, jump-to-message) уже делаются без глушения — обработчик
//     `onScroll` спроектирован это переживать (реагирует на `goingUp`, а
//     запись из `restore()` всегда двигает `scrollTop` ВНИЗ при вставке
//     сверху). Заводить глушение только для этого одного вызывающего было бы
//     новой, ничем не подкреплённой асимметрией — адаптер просто пишет
//     `scrollTop` напрямую;
...
//     `_useReflow` (сигнатура и вызовы не менялись, значение по-прежнему
//     передаётся насквозь из `restore()`/`_restore()`/`setScrollTop()`);
//     аналогично в `_restore()` деструктурированный `scrollable` в tweb тоже
//     нигде не используется в теле функции (вызовы идут через `this.setScrollTop`,
//     который сам обращается к `this.scrollable`) — под `noUnusedLocals`
//     переименован в `_scrollable`, значение не читается ни там, ни там;
//   • поля, не инициализируемые в конструкторе (`scrollHeight`,
//     `scrollHeightMinusTop`, `scrollTop`, `scrolledToEnd`, `clientHeight`,
//     `elements`), помечены `!` — `strictPropertyInitialization` включён у
//     нас (в tweb `strict` выключен), рантайм-семантика не меняется;
//   • форматирование (без `;`, отступы/пробелы в `{ }`) — под `.oxlintrc.json`
//     этого репозитория, логика не менялась ни на строку.
...
// См. пояснение в шапке файла — минимальный локальный аналог
// `ScrollableContextValue` (tweb `components/scrollable2.tsx`), без JS-thumb.
export interface ScrollSaverTarget {
...
export default class ScrollSaver {
   ...
   */
  constructor(
    private scrollable: ScrollSaverTarget,
  ...
  private onRestore(_useReflow?: boolean) {
  ...
  public _restore(useReflow?: boolean) {
    const { scrollHeightMinusTop: previousScrollHeightMinusTop, scrollable: _scrollable } = this
