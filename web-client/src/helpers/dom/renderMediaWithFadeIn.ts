...
//      закрыло подложку.
//
// ЧТО РАСХОДИТСЯ С НАШИМ React-СЛОЕМ (и почему расходится именно он, а не
// порт): `components/messages/useBlurThumb.ts` подложку НЕ снимает вовсе
// (комментарий там же). Здесь оригинал восстановлен как есть —
// `UNMOUNT_THUMBS` в tweb константа-`true`, вырожденная ветка `false` не
// портирована (мёртвый код).
//
