...
4. Завести в `fireAroundAnimation` полную развилку оригинала :1484-1519 —
   вместе с `warmUpDownload` (:1495), которого сейчас в этой ветке НЕТ и быть
   не может: у оригинала он нужен потому, что кадры рисует генерик, а
   каталожные файлы иначе никто не запросит; мы же играем сам каталожный
   эффект, и его файлы тем же тиком качают `wrapSticker`/`wrapStickerAnimation`
   через тот же модульный кэш (`wrappers/stickerContent.ts`). Как только
   появится генерик, каталожные файлы перестанут запрашиваться кликом — и
   `warmUpDownload` в этой точке станет обязательным.
...
докачиваются — их запрашивают `wrapSticker` и `wrapStickerAnimation` тем же
кликом, через общий кэш `wrappers/stickerContent.ts`, — поэтому следующий клик
по той же реакции играет уже нормально.
