...
- **Корень бандла не владеет проводкой.** Файл, неимпортируемый в тестах (точка входа
  воркера `src/core/worker.ts`, `src/client/bootstrap.ts`, корень колонки
  `src/components/Chat.tsx`), содержит только сборку зависимостей и делегирование в
  импортируемый модуль.
- **Критерий приёмки — не только для задач, ВЫНОСЯЩИХ логику из корня бандла.** Любая
  строка проводки в таком файле (создание порта/подписки, регистрация RPC-хендлеров,
  подписка на отключение, ретрансляция кадра — необязательно появившаяся только что
  выносом) обязана либо иметь тест, который краснеет на её удалении/порче, либо быть
  явно помечена комментарием ПРЯМО У НЕЁ как сознательно непокрытая, с причиной.
  «Уже давно так работает и покрыто интеграционно» — не довод: `registerManagers(smp,
  registry)` в `bind()` жила без единого теста на сам факт вызова с момента введения
...
- Мутация обязана краснеть — приводить реальный вывод vitest, не пересказывать.
  Пример: `src/core/workerCore.ts` (`createWorkerCore().bind`) +
  `src/core/workerCore.test.ts` — тест зовёт настоящий `bind()` через фейковые
  эндпоинты и краснеет на порче `smp.onAny(...)`, на замене
  `smp.setOnPortDisconnect(...)` на no-op и на замене `registerManagers(smp, registry)`
  на `void registerManagers`. Пример непокрытой-но-помеченной строки — ветка
  DNP-моста `if (fileDownload) { ep.addEventListener(...) }` там же: комментарий рядом
