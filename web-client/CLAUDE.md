...
- **Корень бандла не владеет проводкой.** Файл, неимпортируемый в тестах (точка входа
  воркера, `bootstrap`, корень колонки), содержит только сборку зависимостей и
  делегирование в импортируемый модуль.
- **Критерий приёмки задачи, выносящей логику из корня бандла:** доказать мутацией не
  только порчу тела вынесенной функции, но и **удаление строки её вызова**. Обе мутации
  обязаны краснеть. Пример: `src/core/workerCore.ts` (`createWorkerCore().bind`) +
  `src/core/workerCore.test.ts` — тест зовёт настоящий `bind()` через фейковые
  эндпоинты и краснеет и на порче `smp.onAny(...)`, и на замене
  `smp.setOnPortDisconnect(...)` на no-op.
