...
| предпросмотр стикера по зажатию в ленте | входа не было вовсе | `attachStickerViewerListeners` на скроллер ленты (tweb bubbles.ts:1328-1336: `listenTo: this.scrollable.container`, `findTarget` по `.attachment.media-sticker-wrapper`/`.media-gif-wrapper`/`.poll-option-sticker`). Сам просмотрщик у нас ПОРТИРОВАН и живой — `components/stickers/useStickerViewer.ts` (подключён в `StickersHelper.tsx:68`, `StickerSetModal.tsx:170`, `StickersTab.tsx:145`, `StickersSearchTab.tsx:248`), к ленте он просто не привязан. Хук React, а лента императивная, — поэтому это не одна строка, а свой заход: либо ванильный слушатель рядом с `onContainerClick`, либо привязка со стороны `VanillaFeed` |
