...
- **Норма: корень бандла не владеет непокрытой проводкой.** Применимость определяется
  РОЛЬЮ файла, а не тем, импортируется ли он тестами — та же роль встречается и у
  импортируемых файлов (`bootstrap.ts`, теперь и `workerCore.ts`). Роль: файл,
  который поднимает среду (воркер, вкладку, страницу) и устанавливает проводку —
  порт/подписку, RPC-хендлер, подписку на отключение, ретрансляцию кадра. Любая
  строка такой проводки обязана либо иметь тест, который краснеет на её удалении/
  порче, либо быть явно помечена комментарием ПРЯМО У НЕЁ как сознательно
  непокрытая, с причиной. «Уже давно так работает и покрыто интеграционно» — не
  довод: `registerManagers(smp, registry)` в `bind()` жила без единого теста на сам
  факт вызова с момента введения RPC-моста, пока весь прогон (177 файлов/1170
  тестов) не остался зелёным на `void registerManagers` вместо неё — молчаливый
  мёртвый RPC-мост для всех 32 менеджеров сразу. Тот же класс дыры нашёлся и в
  точке монтирования `core.start()`: вырезание `if('onconnect' in g){…}else{bind(g)}`
  красило 0 тестов при зелёном полном прогоне (177 файлов/1173 теста) — см.
  `workerCore.test.ts`.
- **Критерий приёмки действует для ЛЮБОЙ проводки в файле этой роли — не только
  для той, что появилась в рамках задачи по выносу логики из корня бандла.**
  Старая, годами лежавшая без теста строка подчиняется норме ровно так же, как
  новая: дата появления строки не освобождает её от требования теста или пометки.
- Мутация обязана краснеть — приводить реальный вывод vitest, не пересказывать.
  ...
  фейковые эндпоинты (для `start()` — через реальный глобальный скоуп happy-dom,
  куда попадает ветка `bind(g)`, т.к. `'onconnect' in self` там false) и краснеет
  на порче `smp.onAny(...)`, на замене `smp.setOnPortDisconnect(...)` на no-op, на
  замене `registerManagers(smp, registry)` на `void registerManagers` и на вырезании
  блока `onconnect`/`bind` из `start()`. Пример непокрытой-но-помеченной строки —
  ветка DNP-моста `if (fileDownload) { ep.addEventListener(...) }` там же:
  комментарий рядом объясняет, что она активна только при DNP-ON и вне периметра
  теста.

**Статус по файлам этой роли:**
- `src/core/worker.ts` — соответствует. Сам файл (`createWorkerCore()` /
  `core.start()`) физически недостижим из vitest — это точка входа воркера,
  запускаемая при импорте модуля браузером, а не библиотечный экспорт; обе строки
  помечены комментарием у себя с этой причиной. Вся проводка, которую они
  запускают, вынесена в `workerCore.ts` и покрыта поведенческими тестами
  `workerCore.test.ts` (`bind()` и `start()`).
- `src/client/bootstrap.ts` — соответствует, но НЕ по формуле «только сборка и
  делегирование»: файл содержит ~25 строк реальной логики Web Locks
  (`attachLock`). Она импортируется и тестируется напрямую —
  `bootstrap.lock.test.ts:114-127` проверяет, что `startClient()` реально вызывает
  `attachLock`, а не только что функция где-то существует.
- `src/components/Chat.tsx` — известное исключение и долг. 1483 строки, ~18
  `useEffect`, ни один тест её не импортирует. Не переписывать ради самой нормы
  прямо сейчас; при следующем содержательном касании файла — приводить затронутую
  проводку в соответствие (тест либо пометка с причиной), а не расширять
  непокрытую площадь дальше.
